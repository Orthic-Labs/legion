...
for(const bundle of ['designer','writing']){
  ...
  for(const record of manifest.files){const path=resolve(root,'skills',bundle,record.path);const sourcePath=resolve(root,'..',bundle,record.path);const sourceBytes=readFileSync(sourcePath);const output=transformSkillText(sourceBytes.toString('utf8'),{bundle,path:record.path,profile:'audit',rules:[{from:'Adrian',to:'the approving human'}]});writeFileSync(path,output);record.sourceDigest=digestBytes(sourceBytes);record.outputDigest=digestBytes(Buffer.from(output));record.transformations=[...new Set([...(record.transformations??[]),'package-relative-links','audit-capability-strip'])];}
