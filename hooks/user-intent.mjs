// Read Adrian's recent instructions and decide, in the hook, whether a block is
// already authorized.
//
...
// one session, for things Adrian had explicitly asked for.
//
...
// System-injected turns arrive with `role: user` but are not Adrian typing.
 ...
 * Adrian already authorized the work? True only on an explicit recent directive
 * with no later hold.
 */
...
// Separate set, NOT authorization: detects Adrian's no-deferral tone ("no
