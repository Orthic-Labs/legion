...
// push` to Adrian's own remotes was worded "irreversible-in-effect" and passed
...
// Ported from enforce_continue_intent.py: Adrian phrases corrections as
...
// AND Adrian issued an explicit no-deferral directive in a recent turn
...
export function toolUseAfterLastUser(transcriptText) {
  ...
  for (let i = lines.length - 1; i >= 0; i -= 1) {
    ...
    if (blocks.some((b) => b?.type === 'tool_result')) continue; // harness echo, not Adrian
    lastUser = i;
    break;
  }
...
export function evaluateStopShape(finalText, { pushes = 0, recorded = false, escalated = false, authorized = false, authorizedEvidence = null, explicitDirectiveEvidence = [], continueIntentEvidence = null, continueToolsUsed = false } = {}) {
  ...
  if (HARD_BLOCKER.test(finalText)) {
    ...
    if (!authorized && isPushGateLaundering(finalText)) {
      return {
        ...
        instruction: 'Your blocker gate is an ordinary git push, worded as reserved. A normal push of requested work to Adrian\'s own remotes is pre-approved: using the word "irreversible" does not make it publication or destruction. Only a --force push or a history rewrite on a shared branch is genuinely reserved. Push it and report the receipt.',
      };
    }
  ...
  // D-5: a scope cut proposed after Adrian explicitly said not to defer.
  if (explicitDirectiveEvidence.length > 0) {
    ...
    if (scopeCut) {
      return {
        block: true,
        shape: 'scope-cut',
        instruction: `Adrian issued an explicit no-deferral directive ("${explicitDirectiveEvidence[0].slice(0, 80)}") and this reply proposes a scope cut or deferral (detected: "${scopeCut}"). Execute the literal request. If genuinely blocked, report "tried [X]: [error]. trying [Y]." and keep moving — do not fork or defer.`,
      };
    }
  }
  ...
  if (continueIntentEvidence && !ACTION_DONE_RE.test(prose) && !CONTINUE_BLOCKER_RE.test(prose)
    ...
    return {
      ...
      instruction: `Adrian's latest message ("${continueIntentEvidence}") is an instruction, not an invitation to explain and stop. Do the work now, or state a hard blocker with the exact missing input. Do not end on "we can/should/I will" when the missing work is safe to do.`,
    };
  }
