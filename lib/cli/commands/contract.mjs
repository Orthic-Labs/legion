...
function contractSeal(argv, { stdout, env, cwd }) {
  ...
  if (!agent) {
    throw new LegionError(
      'contract seal requires --agent <agentId>: the seal carries kernel-observed authority, so it must name an agent identity the host already observed (SubagentStart). There is no --authority flag by design.',
  ...
  const observed = bindings.get({ adapter, sessionId, agentId: agent });
  if (!observed) {
    throw new LegionError(
