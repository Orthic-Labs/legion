...
export async function runVerify(argv, { stdout, stderr, env, cwd,host,core=null }) {
  ...
  if (!existsSync(factsPath) || !statSync(factsPath).isFile()) {
    throw new NemesisError(`verify facts artifact missing: ${factsPath}`, { code: 'VERIFY_RUN_ARTIFACT_MISSING', exitCode: EXIT.USAGE });
  }
  const planPath=runDirectory?join(supplied,'plan.json'):null;if(planPath&&(!existsSync(planPath)||!statSync(planPath).isFile()))throw new NemesisError(`verify plan artifact missing: ${planPath}`,{code:'VERIFY_RUN_ARTIFACT_MISSING',exitCode:EXIT.USAGE});
