...
export function runHooks(argv, { stdout, stderr, cwd }) {
  ...
  if (!action || !['install', 'uninstall'].includes(action)) {
    throw new LegionError('hooks requires install|uninstall [--pre-commit|--pre-push]', { code: 'USAGE', exitCode: EXIT.USAGE });
  }
  stdout.write(`${JSON.stringify({
    schemaVersion: 1, kind: 'legion-hooks', action, target: target ?? null,
