...
const IGNORE_ENTRIES = ['.nemesis/', '.audit/'];
...
export function runInit(argv, { stdout, stderr, cwd }) {
  const parsed = parseArgs({
    ...
    options: { write: { type: 'boolean' }, dryRun: { type: 'boolean' } },
    strict: false,
  });
  ...
  const configPath = resolve(root, 'nemesis.config.json');
  const gitignorePath = resolve(root, '.gitignore');
  const baselinePath = resolve(root, '.nemesis', 'baseline.json');
