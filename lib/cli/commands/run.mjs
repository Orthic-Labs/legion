...
function requireExactTaskBudget({ cwd, env, contract, version, task, digest }) {
  ...
  try {
    ...
    const taskSeal = new TaskBudgetSealStore({ root: taskBudgetRoot(cwd), keyRing }).require(contract, task);
...
function runClose(argv, { stdout, env, cwd, receiptStoreFactory }) {
  ...
  const completion = disposition === 'complete'
    ? evaluateCompletion({ runId: existing.runId, taskId: existing.taskId, claimedLevel: 'signoff', touchedPaths, contractId: existing.contractId, contractVersion: existing.contractVersion, contractDigest: existing.contractDigest }, { policy: new PolicyEngine(loadPolicy()), receiptStore, budgetStore: exactBudget?.store ?? null })
