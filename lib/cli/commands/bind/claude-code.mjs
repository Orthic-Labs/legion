...
const AGENT_FILES = ['sage.md', 'alchemist.md', 'seer.md', 'covenant-seat.md'];
...
export function targets(root) {
  const agentTargets = AGENT_FILES.map((file) => ({
    path: join(root, '.claude', 'agents', file),
    kind: 'whole',
    reason: `install ${file} doctrine agent`,
    content: doctrineText(file),
  }));
