...
export async function runAudit(argv, { stdout, stderr, env, cwd, host, core=null }) {
  let parsed;try{parsed = parseArgs({ args: argv, allowPositionals: true, strict: true,options:{out:{type:'string'},'cortex-out':{type:'string'},only:{type:'string',multiple:true},skip:{type:'string',multiple:true},'plan-only':{type:'boolean'},quiet:{type:'boolean'},url:{type:'string'},surfaces:{type:'string'},'visual-spec':{type:'string'},'visual-baselines':{type:'string'},width:{type:'string'},height:{type:'string'}} });}catch(error){throw new NemesisError(error.message,{code:'USAGE',exitCode:EXIT.USAGE});}
  ...
  stdout.write(`${JSON.stringify({kind:'nemesis-audit-run',outDir:result.artifacts.runDir,report:result.artifacts.report?.path??null,incomplete:result.report?.incomplete??true})}\n`);
