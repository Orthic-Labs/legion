...
function namingBindings(root) {
  ...
  return { gemini, codex: { status: codexLegacy.length ? 'legacy-present' : codexText ? 'canonical' : 'absent', legacy: codexLegacy } };
}
...
export async function runDoctor(argv, { stdout, stderr, env, cwd, host }) {
  ...
  const report = {
    ...
    gaps: [
      ...
      ...([namingBindingState.gemini, namingBindingState.codex].every(({ status }) => !['legacy-present', 'invalid'].includes(status)) ? [] : [{ kind: 'naming-migration-pending', detail: namingBindingState }]),
    ],
    commands: [
      ...
      ...([namingBindingState.gemini, namingBindingState.codex].every(({ status }) => !['legacy-present', 'invalid'].includes(status)) ? [] : ['Run legion bind --write after reviewing reported legacy or conflicting MCP bindings.']),
    ],
  };
