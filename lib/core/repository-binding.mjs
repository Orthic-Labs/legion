...
async function files(root, current = root, output = []) {
  for (const entry of await readdir(current, { withFileTypes: true })) {
    if (['.git', 'node_modules', '.nemesis'].includes(entry.name)) continue;
...
export async function bindRepository(root, { revision = null } = {}) {
  ...
  return Object.freeze({ schemaVersion: 1, root: absoluteRoot, repositoryRevision: revision, dirtyOverlayDigest: `sha256:${hash.digest('hex')}`, fileCount: names.length, digest: digest({ revision, names }) });
}
