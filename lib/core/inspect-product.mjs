...
export async function inspectProduct(options, host) {
  ...
  const releaseContract=options.releaseContract?.kind==='nemesis-release-contract'?options.releaseContract:mergeReleaseContract({...options.releaseContract,binding:options.binding??null});
  ...
  return { schemaVersion: 1, kind: 'nemesis-product-inspection', portfolio, components, stacks, externalSystems,releaseContract,productContext, journeys, baseline, capabilities, claimImpact, scenarios,reportSkeleton };
}
