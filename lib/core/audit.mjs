...
const write=(store,path,kind,binding,value)=>store.writeJson({path,kind,producer:'nemesis.core',schemaVersion:1,binding,value});

export async function audit(options,host){
  const root=resolve(options.root);const runDir=resolve(root,options.outDir??'.audit/nemesis-run');
