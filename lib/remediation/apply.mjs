...
// around application, not the mutation. Nemesis never writes to an audited
...
// (in Legion: Seer proposes, Alchemist mutates). No filesystem API is imported
...
function requireValidVerification(proposal, verification) {
  if (verification?.kind !== 'nemesis-remediation-verification' || verification.valid !== true) {
...
export async function applyProposal({
  ...
  const body = {
    ...
    kind: 'nemesis-remediation-apply-receipt',
    proposalId: proposal.id,
    ...
    recovery: applied
      ...
      : { command: ['nemesis', 'remediation', 'rollback', '--proposal', proposal.id], description: 'restore the pre-apply fingerprint through the host mutator' },
    rolledBack: false,
    binding,
  };
...
export function buildApplyReceiptSchema() {
  return {
    ...
    properties: {
      ...
      kind: { const: 'nemesis-remediation-apply-receipt' },
