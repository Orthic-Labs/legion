...
export async function createRemediationWorktree({ repo, baseCommit, runId, processRunner }) {
  const path = join(repo.gitCommonDir, 'legion-worktrees', runId);
...
export async function removeRemediationWorktree({ path, processRunner }) {
  const result = await processRunner.run({
    ...
    // Never run from inside the worktree being removed — Windows refuses to
    // delete a directory that is the process cwd. The parent of the worktree
    // is always outside it and exists after removal.
    cwd: dirname(path),
  });
