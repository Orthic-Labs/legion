...
export function buildSurfaceInventory({ artifacts = [], binding = {} } = {}) {
  ...
  return { schemaVersion: 1, kind: 'nemesis-surface-inventory', binding, surfaces, denominatorDigest: id(JSON.stringify(surfaces)), complete: artifacts.every((artifact) => artifact.file), coverageGaps: artifacts.filter((artifact) => !artifact.file).map(() => 'surface-source-missing') };
}
