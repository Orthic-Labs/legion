...
export function joinReachability({ vulnerability, callEvidence, graphScope }) {
  if (!callEvidence || callEvidence.length === 0) {
    ...
    return { ...vulnerability, reachability: 'unknown', graphScope: graphScope ?? null, evidenceRefs: [], trace: [], limitations: ['missing-qualified-call-evidence'] };
  }
  ...
  const trace=callEvidence.map(({id,from,to,provider,confidence})=>({id,from,to,provider,confidence}));
  if (reached) return { ...vulnerability, reachability: 'reachable', graphScope, evidenceRefs: callEvidence.map((e) => e.id ?? null).filter(Boolean), trace };
  if (possibly) return { ...vulnerability, reachability: 'possibly-reachable', graphScope, evidenceRefs: callEvidence.map((e) => e.id ?? null).filter(Boolean), trace };
  // Every observed call site was examined and none reached the vulnerable sink.
  return { ...vulnerability, reachability: 'not-reached-in-observed-graph', graphScope, evidenceRefs: callEvidence.map((e) => e.id ?? null).filter(Boolean), trace, limitations:['observed graph is not proof of safety'] };
}
