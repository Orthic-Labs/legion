...
export function validateCommercialLenses(root) {
  ...
  for (const record of records) {
    if (record.availability !== 'available' || record.targetType !== 'content' || record.targetRef !== 'registry/routing/domains.json') findings.push({ code: 'lens-availability', lensId: record.id, detail: 'lens must bind packaged domain routing' });
