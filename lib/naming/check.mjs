...
function semanticIssues(root, registry) {
  ...
  if (JSON.stringify([...AUTHORITY_ID].sort()) !== JSON.stringify(runtimeExpected)) issues.push({ path: 'packages/contracts/enums.mjs', reason: 'runtime authority set differs from naming registry' });
  if (JSON.stringify([...ROSTER_ROLE_IDS].sort()) !== JSON.stringify(['alchemist', 'oracle', 'sage'])) issues.push({ path: 'lib/roster/index.mjs', reason: 'runtime roster differs from naming registry' });
  ...
  if (!packageJson.files.includes('packages/oracle/')) issues.push({ path: 'package.json', reason: 'published files omit canonical Oracle package' });
  ...
  if (!packageManifest.allowlistedTopLevel.includes('packages/oracle/')) issues.push({ path: 'MANIFEST.package.json', reason: 'package manifest omits canonical Oracle package' });
  ...
  for (const { path, declaration, valuePattern = /['"]([^'"]+)['"]/g, expected: values } of [
    { path: 'packages/context/lib/context.mjs', declaration: /const AUTHORITIES\s*=\s*new Set\((\[[^\]]+\])\)/, expected: runtimeExpected },
    { path: 'packages/arcane/lib/architecture-event-store.mjs', declaration: /const ACTOR_ROLES\s*=\s*new Set\((\[[^\]]+\])\)/, expected: ['alchemist', 'covenant', 'host', 'legion', 'oracle', 'sage', 'worker'] },
    { path: 'packages/arcane/lib/authority-binding-store.mjs', declaration: /const MAP\s*=\s*(\{[^}]+\})/, valuePattern: /:\s*['"]([^'"]+)['"]/g, expected: ['alchemist', 'covenant', 'oracle', 'sage'] },
  ]) {
