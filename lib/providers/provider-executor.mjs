...
async function runLegacyCheck(provider, root, plan, projection, artifacts, host) {
  ...
  const result=await host.processRunner.run({kind:'nemesis-named-check',provider:provider.id,check:provider.runner.check,cwd:root,binding:plan.binding??null,denominator:plan.denominator??null,projection,artifacts});
...
async function runRuntimeScript(provider, root, plan, projection, artifacts, host) {
  ...
  const schema=JSON.parse(await readFile(resolve(import.meta.dirname,'..','..','schemas/provider-result-v1.schema.json'),'utf8'));
