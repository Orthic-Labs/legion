...
- **Decompose/architecture findings route to `architect`** (its full `/covenant` workflow is the
  external design gate) — the loop drives the refactor autonomously, it does not hand `/architect` back to Adrian.
...
the full `/covenant` workflow as its OWN external gate, and it is built to proceed without waiting for Adrian.
...
Do not hand `/architect`+`/covenant` back to Adrian to run manually; that hand-off was the old stall.
...
Older CodeRight asset copies may use `name: repository-auditor`. Public shared invocation is
