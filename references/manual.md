...
the full `/council` workflow as its OWN external gate, and it is built to proceed without waiting for Adrian.
...
Do not hand `/architect`+`/council` back to Adrian to run manually; that hand-off was the old stall.
