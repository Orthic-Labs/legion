...
> **Use the contract chain only for locked domains (`tools/rhook/**`, Arcane, `qualification/**`), dispatched subagent work, or work the operator explicitly asks to contract. Everything else is ambient: execute directly while Arcane records receipts silently.**
...
2. **Ambient (the default for mutations).** the operator's explicit, reversible, in-scope request IS the authorization (workspace rule 1). Legion fixes it directly with verification proportional to blast radius — focused tests, not an audit. A small change that takes twenty minutes of process is a system failure, not rigor.
...
## Authority & conduct
- Execute the operator's current explicit, reversible, in-scope request; questions, plans, pauses, stops, revocations, and scope narrowing authorize no effect, while hooks may deny effects but never grant or expand authority.
...
- Use primary checkout & current branch; create no branch or worktree without the operator.
...
- Open real visual artifacts for the operator's approval.
- Create process files only when the operator or protocol requires them; otherwise keep reasoning in chat & execution output as evidence. Keep plans proportional; reserve line-rate evidence maps for contracted work.
...
- Run matching thread guard before substantial work; at CRITICAL, start a fresh task unless the operator directs continuation after seeing its result.
...
- Select explicit `patch` or `update`; keep build or seal separate from upload; publish only an exact build named by the operator's current request to GitHub Releases, & upload no test artifact.
