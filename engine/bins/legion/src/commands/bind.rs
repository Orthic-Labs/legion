...
pub fn run(args: BindArgs) -> CommandResult {
    ...
    let failed = args.write && plans.iter().any(|p| p.targets.iter().any(|t| !t.conflicts.is_empty()));
...
fn detect_harnesses(root:&Path)->Vec<String>{if root.join(".gemini").exists()||root.join("GEMINI.md").exists(){vec!["gemini".into()]}else{Vec::new()}}
...
fn migrate_legacy_config(raw:&str)->(String,Option<Vec<Value>>){let(assurance,conf)=assurance_migration(raw);if conf.is_some(){return(raw.into(),conf)}let managed=assurance.contains(TOML_START)&&assurance.contains(TOML_END);let count=assurance.lines().filter(|l|l.trim()=="[mcp_servers.legion]").count();let aliases=["forge","sorcerer","seer","sentinel"];let legacy=aliases.iter().any(|id|assurance.lines().any(|l|l.trim()==format!("[agents.{id}]")));let generated=!managed&&assurance.contains("# Generated by Legion bind.")&&legacy_headers().iter().any(|h|assurance.lines().any(|l|l.trim()==h));if(!managed&&!legacy&&!generated)||(managed&&count<2){return(assurance,None)}let mut out=Vec::new();let mut drop=false;let mut inside=false;for line in assurance.lines(){if line.contains(TOML_START){inside=true}if line.contains(TOML_END){inside=false;drop=false;out.push(line);continue}let t=line.trim_start();if t.starts_with('['){if let Some(i)=t.find(']'){let h=&t[..=i];drop=!inside&&(if managed{h=="[mcp_servers.legion]"}else{legacy_headers().contains(h)});}}if !drop{out.push(line)}}(format!("{}\n",out.join("\n").trim()),None)}
