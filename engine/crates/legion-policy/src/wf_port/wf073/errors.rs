...
//! scoped to the codes this chunk's modules actually raise. The full 40+
//! code taxonomy already exists natively at `legion_arcane::ArcaneError`
//! (`engine/crates/legion-arcane/src/error.rs`); `legion-policy` does not
//! depend on `legion-arcane` today, so — same as wf068's local stand-in at
//! `wf_port::wf068::errors::ArcaneError` — this is a minimal, code-compatible
//! type scoped to this chunk rather than a shared import across owned
//! porting directories.
