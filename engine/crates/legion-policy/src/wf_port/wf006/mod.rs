...
//! for per-file status, the dependency patch this module would like once
//! wired in (to replace the local `canonical` module with
//! `legion-contracts::canonical` + `sha2`/`hmac`), and the scope left in
//! `preeffect_gate` (utilities only — the `PreEffectGate` class itself is
