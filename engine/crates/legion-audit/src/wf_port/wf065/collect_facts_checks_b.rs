//! Port of the SECOND HALF of `tools/audit/collect-facts.mjs`'s `buildChecks()`
//! check bodies (source order): `cargo_audit`, `cargo_deny`,
//! `cargo_unused_deps`, `cargo_unsafe`, `vendored_deps`, `dep_pinning`,
//! `tool_coverage`, `contract_mirror`, `tauri_capabilities`,
//! `apple_platform`, `react_hooks`, `negative_space`, `outdated`,
//! `cargo_outdated`, `binary_pins`, `debt_markers`, `build`.
//!
//! The first half (`repo` .. `js_licenses`) is ported by a sibling module,
//! `collect_facts_checks_a.rs`. Subprocess execution goes through the
//! [`CommandRunner`] trait so every check here is testable with a fake
//! runner instead of a real spawn.
//!
...
//!   latest" network lookup is not ported (this crate's allowed new deps
//!   include `reqwest`, but wiring live HTTP behind a fake-testable trait
//!   for one sub-field was out of this pass's scope). Every pin is still
//!   found, classified `sha256_pinned`/`rolling`, with `latest: null` and
...
use super::collect_facts::{is_generated_or_vendored_path, redact, DetectedStack};

/// Abstraction over spawning a subprocess, so every check below is testable
/// with a fake instead of a real `cargo-audit`/`cargo-deny`/... binary.
pub trait CommandRunner {
    /// Runs `argv[0] argv[1..]` in `cwd`. Mirrors the JS `run(cmd, {cwd,
    /// timeoutMs})` helper's return shape (stdout/stderr/code/duration_ms).
    fn run(&self, argv: &[&str], cwd: &Path, timeout_ms: u64) -> CommandOutput;
    /// Mirrors the JS `which(bin)`: true iff `bin` resolves on PATH.
...
/// One check's result, pre-`execOne` framing (the `check`/`tool`/`tier`/
/// `parallel`/`log` envelope fields are added by `collect_facts_main`, same
/// division of labour as the JS `run()` closure vs. `execOne()`).
...
fn unproven(reason: &str) -> CheckResult {
    CheckResult {
...
fn unproven_absent(reason: &str) -> CheckResult {
    CheckResult {
        status: CheckStatus::Unproven,
        skip_reason: Some(reason.to_string()),
        tool_absent: true,
...
/// `cargo_audit`: RUSTSEC advisories via `cargo audit --json`.
pub fn check_cargo_audit(d: &DetectedStack, root: &Path, runner: &dyn CommandRunner) -> CheckResult {
    if !d.rust {
        return unproven("no Cargo.toml");
    }
    let Some(rust_dir) = &d.rust_dir else {
        return unproven("no Cargo.toml");
    };
    ...
    let r = runner.run(&["cargo", "audit", "--json"], &cwd, 300_000);
    let count = serde_json::from_str::<Value>(&r.stdout)
        .ok()
        .and_then(|j| j.get("vulnerabilities")?.get("count")?.as_i64());
    CheckResult {
...
/// `cargo_deny`: `cargo deny --format json check` — JSON diagnostics stream
/// on stderr (and stdout, concatenated per the JS `(stderr + '\n' +
/// stdout)`), one object per line; counts error/warning-severity ones.
pub fn check_cargo_deny(d: &DetectedStack, root: &Path, runner: &dyn CommandRunner) -> CheckResult {
    if !d.rust {
        return unproven("no Cargo.toml");
    }
    ...
    let rust_dir = d.rust_dir.clone().unwrap_or_default();
    let cwd = root.join(&rust_dir);
    let r = runner.run(&["cargo", "deny", "--format", "json", "check"], &cwd, 300_000);
    ...
    CheckResult {
        ...
        exit_code: Some(r.code),
        findings_count: if count > 0 { Some(count) } else { None },
        raw_log: redact(Some(&format!("{}{}", r.stdout, r.stderr))),
        duration_ms: Some(r.duration_ms),
        ..Default::default()
    }
}
...
/// `cargo_unused_deps`: `cargo machete --with-metadata`. No JSON output;
/// unused crates print one per indented line under each crate header.
pub fn check_cargo_unused_deps(d: &DetectedStack, root: &Path, runner: &dyn CommandRunner) -> CheckResult {
    if !d.rust {
        return unproven("no Cargo.toml");
    }
    ...
    let rust_dir = d.rust_dir.clone().unwrap_or_default();
    let cwd = root.join(&rust_dir);
    let r = runner.run(&["cargo", "machete", "--with-metadata"], &cwd, 180_000);
...
/// `cargo_unsafe`: `cargo geiger --output-format Json --quiet`; sums used
/// unsafe counts across every category of every package.
pub fn check_cargo_unsafe(d: &DetectedStack, root: &Path, runner: &dyn CommandRunner) -> CheckResult {
    if !d.rust {
        return unproven("no Cargo.toml");
    }
    ...
    let rust_dir = d.rust_dir.clone().unwrap_or_default();
    let cwd = root.join(&rust_dir);
    let r = runner.run(&["cargo", "geiger", "--output-format", "Json", "--quiet"], &cwd, 600_000);
        ...
        j.get("packages")
            ...
            .map(|pkgs| {
                pkgs.iter()
                    ...
                    .filter_map(|cat| cat.get("unsafe_").and_then(Value::as_i64))
                    .sum::<i64>()
            })
            .unwrap_or(0)
    });
...
/// `vendored_deps`: enumerate `vendor/**/{package.json,Cargo.toml}` trees
/// from a caller-supplied `git ls-files` listing (the root lockfile audit
/// never sees these).
pub fn check_vendored_deps(d: &DetectedStack, tracked: Option<&[String]>) -> CheckResult {
    if !d.git {
    ...
    {
        let dir = Regex::new(r"/(package\.json|Cargo\.toml)$").unwrap().replace(m, "").to_string();
        ...
        seen.push((dir, m.clone(), has_lockfile));
    }
    let raw_log = seen
        .iter()
        .map(|(dir, _, has_lockfile)| format!("{dir}  lockfile={has_lockfile}"))
        .collect::<Vec<_>>()
        .join("\n");
    let trees: Vec<Value> = seen
        .iter()
        .map(|(dir, manifest, has_lockfile)| json!({"dir": dir, "manifest": manifest, "has_lockfile": has_lockfile}))
        .collect();
    CheckResult {
        status: CheckStatus::Ran,
        command: Some(
        ...
        exit_code: Some(0),
        findings_count: Some(trees.len() as i64),
...
/// `dep_pinning`: unpinned git deps in `package.json`/`Cargo.toml`.
pub fn check_dep_pinning(d: &DetectedStack, root: &Path, tracked: Option<&[String]>) -> CheckResult {
    if !d.git {
        return unproven("needs git ls-files");
    }
    let Some(tracked) = tracked else {
    ...
    CheckResult {
        ...
        exit_code: Some(0),
        findings_count: Some(offenders.len() as i64),
...
/// `tool_coverage`: what the configured linters/compilers are told to
/// IGNORE (`.eslintignore`, flat-config `ignores:`, `tsconfig` include/
/// exclude) plus top-level code dirs from a caller-supplied tracked list.
pub fn check_tool_coverage(d: &DetectedStack, root: &Path, tracked: Option<&[String]>) -> CheckResult {
...
/// `contract_mirror`: Tauri IPC surface diff — `generate_handler![]` names
/// vs. frontend `invoke("...")` literals.
pub fn check_contract_mirror(d: &DetectedStack, root: &Path, tracked: Option<&[String]>) -> CheckResult {
    if !d.tauri || !d.git {
        return unproven(if d.tauri { "needs git ls-files" } else { "no src-tauri/tauri.conf.json" });
    }
    let Some(tracked) = tracked else {
        return unproven("needs git ls-files");
    };
    ...
    for f in tracked.iter().filter(|f| {
        !f.starts_with("src-tauri/")
            ...
            && super::collect_facts::classify_file(f).as_str() != "test"
    }) {
        let txt = read_to_string(root, f);
    ...
    CheckResult {
        ...
        exit_code: Some(0),
        findings_count: Some((uncalled.len() + unregistered.len()) as i64),
        meta: Some(json!({
...
/// `tauri_capabilities`: Tauri v2 capability-file permission surface —
/// flags broad/dangerous grants plus counts exposed `#[tauri::command]`s.
pub fn check_tauri_capabilities(d: &DetectedStack, root: &Path, tracked: Option<&[String]>) -> CheckResult {
    if !d.tauri {
    ...
    if d.git {
        if let Some(tracked) = tracked {
            ...
            for f in tracked
                ...
                .filter(|f| f.starts_with("src-tauri/") && f.ends_with(".rs") && !is_generated_or_vendored_path(f))
            {
                exposed += cmd_re.find_iter(&read_to_string(root, f)).count() as i64;
            }
        }
    }
    let meta = json!({
    ...
    CheckResult {
        ...
        exit_code: Some(0),
        findings_count: Some(broad.len() as i64),
...
// ---------------------------------------------------------------------
// apple_platform (usage-description / ATS / entitlement findings only —
// see module doc-comment for the keyboard-inventory gap)
// ---------------------------------------------------------------------
...
/// `apple_platform`: Info.plist/entitlements/project.yml consistency for
/// Apple-platform targets — missing usage descriptions, ATS weakening, a
/// committed debug entitlement.
pub fn check_apple_platform(root: &Path, tracked: Option<&[String]>) -> CheckResult {
    let Some(tracked) = tracked else {
    ...
    CheckResult {
        ...
        findings_count: Some(findings_len as i64),
        raw_log: serde_json::to_string_pretty(&meta).ok(),
        meta: Some(meta),
        duration_ms: Some(0),
        ..Default::default()
...
/// `react_hooks`: React project ⇒ `eslint-plugin-react-hooks` configured?
pub fn check_react_hooks(d: &DetectedStack) -> CheckResult {
    let has_react = d.node && super::collect_facts::has_dep(d.pkg.as_ref().unwrap_or(&Value::Null), "react");
    if !d.node || !has_react {
        return unproven("not a React project");
    }
    let present = super::collect_facts::has_dep(d.pkg.as_ref().unwrap_or(&Value::Null), "eslint-plugin-react-hooks");
    let meta = json!({
...
/// `negative_space`: absence of README/LICENSE/.gitignore/CI/lockfile/tests
/// plus (with a tracked-file list) test-skew, large-binary, CI-advisory-
/// gate and Rust-unsafe inventory.
pub fn check_negative_space(d: &DetectedStack, root: &Path, tracked: Option<&[String]>) -> CheckResult {
    ...
    let has_ci = d.workflows || exists_any(&[".gitlab-ci.yml", "azure-pipelines.yml", ".circleci"]);
    if !has_ci {
        missing.push("CI");
    }
    if d.node && !exists_any(&["package-lock.json", "pnpm-lock.yaml", "yarn.lock"]) {
        missing.push("lockfile");
    }
    let has_test_script = d
        .pkg
        .as_ref()
    ...
    if d.git {
        if let Some(tracked) = tracked {
            ...
            for f in tracked {
                ...
                let key = if super::collect_facts::classify_file(f).as_str() == "test" { "test" } else { "code" };
            ...
            if d.rust {
                ...
                for f in tracked
                    .iter()
                    .filter(|f| f.ends_with(".rs") && !is_generated_or_vendored_path(f) && super::collect_facts::classify_file(f).as_str() != "test")
                {
    ...
    CheckResult {
        ...
        exit_code: Some(0),
        findings_count: Some(missing.len() as i64),
...
/// `outdated`: `<pkgMgr> outdated --json`, plus a majors-behind count.
pub fn check_outdated(d: &DetectedStack, root: &Path, runner: &dyn CommandRunner) -> CheckResult {
    if !d.node {
        return unproven("no package.json");
    }
    let cmd = format!("{} outdated --json", d.pkg_mgr);
    let r = runner.run(&[d.pkg_mgr, "outdated", "--json"], root, 180_000);
    if looks_missing(&r) {
        return unproven_absent(&format!("{} not available", d.pkg_mgr));
    }
    let mut total = None;
    let mut majors = 0i64;
    if let Ok(Value::Object(entries)) = serde_json::from_str::<Value>(if r.stdout.is_empty() { "{}" } else { &r.stdout }) {
        total = Some(entries.len() as i64);
...
fn looks_missing(r: &CommandOutput) -> bool {
    // Mirrors `looksMissing`: caller passes ENOENT-shaped output for an
    // absent binary; treated identically to `collect_facts::looks_missing`.
    super::collect_facts::looks_missing(&r.stdout, &r.stderr)
}
...
pub fn check_cargo_outdated(d: &DetectedStack, root: &Path, runner: &dyn CommandRunner) -> CheckResult {
    if !d.rust {
        return unproven("no Cargo.toml");
    }
    let Some(rust_dir) = &d.rust_dir else {
    ...
    let cwd = root.join(rust_dir);
    let r = runner.run(&["cargo", "outdated", "--format", "json", "--root-deps-only"], &cwd, 300_000);
    let mut total = None;
    let mut majors = 0i64;
...
/// `binary_pins`: scan tracked build/packaging scripts for hardcoded
/// artifact-download URLs; a 64-hex literal within ±10 lines counts as an
/// integrity pin. Upstream-latest resolution (GitHub/npm) is not ported —
/// every pin's `latest`/`stale` stay `null`, matching the JS's own
/// "unresolved => MANUAL-CHECK, never silently clean" rule.
pub fn check_binary_pins(d: &DetectedStack, root: &Path, tracked: Option<&[String]>) -> CheckResult {
    if !d.git {
        return unproven("needs git ls-files");
    }
    let Some(tracked) = tracked else {
    ...
    CheckResult {
        ...
        exit_code: Some(0),
        findings_count: Some((stale + unpinned + manual) as i64),
...
pub fn check_debt_markers(d: &DetectedStack, root: &Path, runner: &dyn CommandRunner) -> CheckResult {
    if !d.git {
        return unproven("debt-marker scan needs a git repo");
    }
    let argv = [
        "git", "grep", "-nIE",
        "(ponytail:|\\b(TODO|FIXME|HACK|XXX)\\b)",
        "--",
        ".",
        ":(exclude)*lock.yaml",
        ":(exclude)*lock.yml",
    ...
    let cmd = argv.join(" ");
    let r = runner.run(&argv, root, 180_000);
    ...
    CheckResult {
        ...
        command: Some(cmd),
        exit_code: Some(0),
        findings_count: Some(lines.len() as i64),
...
// ---------------------------------------------------------------------
// build (SEQUENTIAL — runs alone, after the pool; see collect_facts_main)
// ---------------------------------------------------------------------

/// `build`: `<pkgMgr> run build` or `cargo build`.
pub fn check_build(d: &DetectedStack, root: &Path, runner: &dyn CommandRunner) -> CheckResult {
    let (argv, cwd): (Vec<&str>, std::path::PathBuf) = if d.build_script {
        (vec![d.pkg_mgr, "run", "build"], root.to_path_buf())
    } else if d.rust {
        (vec!["cargo", "build"], root.join(d.rust_dir.as_deref().unwrap_or(".")))
    } else {
        return unproven("no build script / Cargo.toml");
    };
    let cmd = argv.join(" ");
    let r = runner.run(&argv, &cwd, 600_000);
    let warn_re = Regex::new(r"(?i)\bwarn(ing)?\b").unwrap();
    let warns = format!("{}{}", r.stdout, r.stderr).split('\n').filter(|l| warn_re.is_match(l)).count();
    CheckResult {
        status: if r.code == 0 { CheckStatus::Ran } else { CheckStatus::Error },
        command: Some(cmd),
        exit_code: Some(r.code),
        findings_count: Some(warns as i64),
        raw_log: redact(Some(&format!("{}{}", r.stdout, r.stderr))),
        duration_ms: Some(r.duration_ms),
        ..Default::default()
    }
...
mod tests {
    ...
    impl FakeRunner {
        fn new() -> Self {
            Self { which: HashMap::new(), outputs: RefCell::new(HashMap::new()), calls: RefCell::new(Vec::new()) }
        }
        fn with_which(mut self, bin: &'static str, present: bool) -> Self {
            self.which.insert(bin, present);
            self
        }
        fn with_output(self, argv0: &str, out: CommandOutput) -> Self {
            self.outputs.borrow_mut().insert(argv0.to_string(), out);
            self
        }
    }

    impl CommandRunner for FakeRunner {
        fn run(&self, argv: &[&str], _cwd: &Path, _timeout_ms: u64) -> CommandOutput {
            let key = argv.join(" ");
            self.calls.borrow_mut().push(key.clone());
            self.outputs.borrow().get(&key).cloned().unwrap_or_default()
        }
        fn which(&self, bin: &str) -> bool {
    ...
    fn cargo_audit_skips_without_cargo_toml() {
        let r = check_cargo_audit(&stack(false), Path::new("."), &FakeRunner::new());
        assert_eq!(r.status, CheckStatus::Unproven);
    ...
    fn cargo_audit_flags_absent_tool() {
        ...
        let r = check_cargo_audit(&stack(true), &dir, &FakeRunner::new().with_which("cargo-audit", false));
    ...
    fn cargo_audit_parses_vulnerability_count() {
        ...
        let runner = FakeRunner::new().with_which("cargo-audit", true).with_output(
            "cargo audit --json",
            CommandOutput { stdout: r#"{"vulnerabilities":{"count":3}}"#.to_string(), code: 0, duration_ms: 12, ..Default::default() },
        );
        let r = check_cargo_audit(&stack(true), &dir, &runner);
        assert_eq!(r.status, CheckStatus::Ran);
    ...
    fn cargo_deny_counts_error_and_warning_diagnostics() {
        ...
        let stderr = r#"{"type":"diagnostic","fields":{"severity":"error"}}
...
"#;
        let runner = FakeRunner::new().with_which("cargo-deny", true).with_output(
            ...
            CommandOutput { stderr: stderr.to_string(), code: 1, duration_ms: 5, ..Default::default() },
        );
        let r = check_cargo_deny(&d, &dir, &runner);
    ...
    fn cargo_unused_deps_counts_indented_non_noise_lines() {
        ...
        let runner = FakeRunner::new().with_which("cargo-machete", true).with_output(
            ...
            CommandOutput { stdout: stdout.to_string(), code: 0, duration_ms: 1, ..Default::default() },
        );
        let r = check_cargo_unused_deps(&d, Path::new("."), &runner);
        assert_eq!(r.findings_count, Some(2));
    }
    ...
    fn cargo_unsafe_sums_used_unsafe_across_packages() {
        ...
        let r = check_cargo_unsafe(&d, Path::new("."), &runner);
        assert_eq!(r.findings_count, Some(3));
    }
    ...
    fn vendored_deps_finds_manifest_trees_with_lockfile_flag() {
        ...
        let r = check_vendored_deps(&stack(true), Some(&tracked));
        assert_eq!(r.findings_count, Some(2));
    ...
    fn dep_pinning_flags_unpinned_npm_git_dep_and_accepts_sha_pinned() {
        ...
        let r = check_dep_pinning(&stack(false), &dir, Some(&tracked));
    ...
    fn negative_space_flags_missing_readme_and_license() {
        ...
        d.workflows = true;
        let r = check_negative_space(&d, &dir, None);
    ...
    fn debt_markers_splits_ponytail_from_todo_and_flags_no_trigger() {
        ...
        let runner = FakeRunner::new().with_output(
            "git grep -nIE (ponytail:|\\b(TODO|FIXME|HACK|XXX)\\b) -- . :(exclude)*lock.yaml :(exclude)*lock.yml :(exclude)*lock.json :(exclude)vendor/** :(exclude)qwik/**",
            CommandOutput { stdout: stdout.to_string(), code: 0, duration_ms: 3, ..Default::default() },
        );
        let r = check_debt_markers(&stack(false), Path::new("."), &runner);
        assert_eq!(r.findings_count, Some(3));
        let meta = r.meta.unwrap();
    ...
    fn build_uses_pkg_mgr_run_build_when_build_script_present() {
        ...
        let runner = FakeRunner::new().with_output(
            "pnpm run build",
            CommandOutput { stdout: "warning: unused import\n".to_string(), code: 0, duration_ms: 7, ..Default::default() },
        );
        let r = check_build(&d, Path::new("."), &runner);
        assert_eq!(r.status, CheckStatus::Ran);
        assert_eq!(r.findings_count, Some(1));
    }
    ...
    fn build_is_unproven_without_build_script_or_rust() {
        let r = check_build(&stack(false), Path::new("."), &FakeRunner::new());
        assert_eq!(r.status, CheckStatus::Unproven);
    }
