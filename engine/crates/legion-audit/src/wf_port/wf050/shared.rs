//! Port of `src/providers/runtime/web/shared.mjs` (chunk wf050 support).
//!
//! These are the untyped JSON helpers every `src/providers/runtime/web/*`
//! module composes with (`finalize`, `exactBinding`, `sameBinding`,
//! `denominator`, `redact`, `sortById`, `canonicalize`/`digest`). `shared.mjs`
//! is itself owned by this chunk's file list, so this is the canonical
//! wf050 copy; chunk wf048 already ported an equivalent scoped to itself
//! (`wf_port::wf048::shared`) before this chunk landed. If the integrator
//! later collapses both onto one shared location, this module and
//! `wf048::shared` should be reconciled at that time.
...
fn opaque(kind: &str, value: &str) -> String {
    ...
    format!("opaque-{}", &hex::encode(hash)[..24])
}
...
/// with stable opaque placeholders. `serde_json::Value` has no bigint,
/// symbol, function or undefined variants, so only the object/array/cycle
/// branches are reachable here; this mirrors the JS behaviour for every
/// value this port can actually construct.
...
const SENSITIVE_SUFFIXES: &[&str] =
    &["apikey", "authorization", "cookie", "password", "passwd", "privatekey", "secret", "secretkey", "token"];

pub fn is_sensitive_key(key: &str) -> bool {
    let normalized = normalized_sensitive_key(key);
    SENSITIVE_KEYS.contains(&normalized.as_str()) || SENSITIVE_SUFFIXES.iter().any(|suffix| normalized.ends_with(suffix))
}
...
pub fn exact_binding(binding: &Value) -> ExactBindingResult {
    ...
    for key in sorted_keys {
        let value = if invalid.contains(&key) {
            ...
            Value::String(opaque("binding", &serde_json::to_string(&canonicalize(&raw)).unwrap_or_default()))
        } else {
...
pub fn denominator(expected: &[String], receipt_ids: &[String], omitted_ids: &[String]) -> DenominatorCounts {
    let mut expected_ids: Vec<String> =
        expected.iter().cloned().collect::<std::collections::BTreeSet<_>>().into_iter().collect();
    expected_ids.sort();
    ...
    let missing: Vec<String> =
        expected_ids.iter().filter(|id| !receipt_set.contains(id) && !omitted_set.contains(id)).cloned().collect();
    DenominatorCounts {
    ...
    if !binding_gaps.is_empty() {
        if let Value::Object(map) = &mut safe_value {
            ...
            for gap in &binding_gaps {
                let full = if gap.starts_with("binding-extra-") { gap.clone() } else { format!("binding-invalid:{gap}") };
