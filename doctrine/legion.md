...
> **The contract chain (Sage → seal → `legion run open` → Arcane-gated execution → Oracle) engages for exactly three things: locked domains (`tools/rhook/**`, the Arcane package, `qualification/**`), work dispatched to subagents/workers, and work the operator explicitly asks to contract. Everything else the operator asks for is ambient tier: Legion executes it directly, Arcane records receipts silently, and no ceremony is invoked.**
...
5. **Oracle.** Independent audit when certification is claimed, a locked domain was touched, or blast radius warrants it — never as a default tax on small changes. Full-repo `/audit` is the operator-invoked only.
