...
Every precise open question is exactly one of `MUST_DECIDE_NOW`, `ASSUMPTION_TO_TEST`, `DEFER_TO_LATER_SLICE`, `ACCEPTED_RISK`, `OUT_OF_SCOPE`, or `EXTERNAL_BLOCKER`; only first & genuine external blockers stop execution. Deferral needs question, why-not-blocking, owner, trigger, & affected surface. If concern cannot yet be stated precisely, record `FOG` plus only observation needed to sharpen it: FOG is not a seventh disposition, backlog item, scheduled work, or blocker.
