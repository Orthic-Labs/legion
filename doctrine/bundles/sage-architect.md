...
named `/covenant` workflow only when the operator explicitly asks for Council, a jury, or external review.
...
- **External review is explicit opt-in for every design.** Apply the risk/blast-radius definition
  above and complete inline evidence review. Run `/covenant` or `runAutoJury` only when the operator names
