...
3. **Escalate** — the disagreement is a genuinely reserved decision or a one-way door: present the
   frozen option set (≤3, with a recommendation) to Adrian. Never present the loop itself as
   progress.
...
best-in-class` are special claims produced only when Adrian explicitly requests a best-shape
...
the decision rests on, or Adrian's explicit reversal. Reopening produces an explicit
...
Adrian with the frozen options. Hours of silent design revision is a system failure of exactly the
...
1. **They are not redundant — they govern a different loop.** The runtime counts **sealed
   ...
   amendment proof; scope expansion requires an authenticated user turn; only Adrian's resume
