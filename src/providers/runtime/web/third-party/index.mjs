...
export function verifyThirdPartyExercise({ binding = {}, configuredProviders = [], applicableProviders = [], exercises = [], applicabilitySource = null, notApplicableReceipts = [], trustedProducers = [], now = null, maxAgeMs = null } = {}) {
  ...
  if (identifiers.some((id) => typeof id !== 'string' || !SAFE_IDENTIFIER.test(id))) return finalize('legion-web-third-party-exercise', { status: 'error', terminal: true, claimLevel: 'external', suppliedOnly: true, networkAttempted: false, binding, denominator: denominator([], []), receipts: [], coverageGaps: ['third-party-identifiers-invalid'] });
  ...
  return finalize('legion-web-third-party-exercise', { status: gaps.length ? 'unproven' : 'pass', terminal: true, claimLevel: 'external', suppliedOnly: true, networkAttempted: false, binding, denominator: counts, receipts, coverageGaps: [...new Set(gaps)].sort() });
}
