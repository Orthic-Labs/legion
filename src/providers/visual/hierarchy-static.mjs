export function analyzeHierarchy({ signals = [], renderedEvidence = null } = {}) {
  ...
  return { provider: 'visual.hierarchy-static', status: findings.length || candidates.length ? 'candidates' : coverageGaps.length ? 'unproven' : 'pass', findings, candidates, coverageGaps, reviewRequired: candidates.length > 0 };
}
