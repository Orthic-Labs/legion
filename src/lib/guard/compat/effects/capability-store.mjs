...
export class CapabilityStore {
  ...
  #legacyIssue(input) { const record = { ...input, advisoryProfile: input.advisoryProfile ?? null, issuedAt: input.issuedAt || stamp(this.#clock), usedCount: 0, status: 'active', revokedReason: null }; this.#memory.set(input.capabilityId, record); return input.capabilityId; }
  issue(input) {
  ...
  get(capabilityId) {
    ...
    return { ...grant, advisoryProfile: grant.advisoryProfile ?? null, status: transition?.state === 'revoked' ? 'revoked' : grant.status, usedCount: transition?.state === 'consumed' ? 1 : grant.usedCount, transition };
  }
  check(capabilityId, ctx = {}) {
    ...
    if (!this.#root) {
      ...
      if ((record.advisoryProfile !== null && !Object.hasOwn(ctx, 'advisoryProfile')) || (Object.hasOwn(ctx, 'advisoryProfile') && !same(record.advisoryProfile, ctx.advisoryProfile))) return fail('ARC_BINDING_MISMATCH', capabilityId, { field: 'advisoryProfile' });
