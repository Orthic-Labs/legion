...
export function editorDiagnostics({ report, runDir }) {
  const findings = (report?.findings ?? []).map((finding) => {
    ...
    return {
      ...
      code: finding.ruleId ?? 'legion.finding',
      message: finding.title ? `${finding.title}${finding.detail ? ` — ${finding.detail}` : ''}` : (finding.detail ?? 'Legion finding'),
      ...
      explanation: { kind: 'legion-finding-explanation', findingId: finding.id ?? null, evidenceRefs: finding.evidenceRefs ?? [] },
      ...
      codeActions: finding.id || finding.ruleId
        ? [...(remediationBound ? [{ title: 'Preview Legion remediation', kind: 'preview', previewOnly: true, proposalId: finding.remediation.id, findingId: finding.id, fingerprint: finding.fingerprint ?? null, patch: finding.remediation.patch }] : []), { title: 'Explain with Legion', kind: 'explain', id: finding.id ?? finding.ruleId }]
        : [],
    };
  });
  const gaps = (report?.coverage_gaps ?? []).filter((gap) => gap.reviewRequired).map((gap) => ({
    schemaVersion: 1, kind: 'legion-editor-diagnostic', file: null,
    range: null, severity: 2, code: 'legion.coverage-gap', message: gap.detail ?? gap.kind ?? 'Incomplete coverage',
    fingerprint: gap.id ?? `gap:${gap.kind}`, findingId: null, family: gap.family ?? null,
    evidenceClass: 'missing', explanation: { kind: 'legion-gap-explanation', gapId: gap.id ?? null },
