// Canonical public library surface for @orthic-labs/legion.
...
export { LEGION_VERSION, LEGION_PACKAGE, LEGION_REPOSITORY } from './version.mjs';
export {
  RunArtifactStore,
  audit,
  bindRepository,
...
export async function loadReport(path, { fs } = {}) {
  ...
  if (report?.kind !== 'repository-audit-report' && !Array.isArray(report?.findings)) throw new TypeError('invalid Legion report');
  return report;
}
...
export {
  ...
  verifyPlanSignature,
  writeAuditPlan,
} from '../../tools/audit/audit-plan.mjs';
