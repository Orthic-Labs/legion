...
export function validateExternalEvidence(evidence, expected = {}) {
  ...
  return { schemaVersion: 1, kind: 'legion-external-evidence-validation', evidenceDigest: evidence ? sha256(evidence) : null, status: gaps.length ? 'unproven' : 'pass', payload: safePayload, gaps: [...new Set(gaps)].sort() };
}
