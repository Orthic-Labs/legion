...
export async function runSkills(argv, { stdout, stderr }, manifests = null) {
  manifests ??= await packagedManifests(); if (argv.includes('--help')) { stdout.write('Usage: legion skills [list|verify] [--source-root <path>] [--bundle <id>] [--publication]\n'); return { exitCode: 0 }; }
  const command = argv[0] ?? 'list'; let sourceRoot = null, publication = false; const bundles = [];
  for (let index = 1; index < argv.length; index++) { if (argv[index] === '--source-root' && argv[index + 1]) sourceRoot = argv[++index]; else if (argv[index] === '--bundle' && argv[index + 1]) bundles.push(argv[++index]); else if (argv[index] === '--publication') publication = true; else { stderr.write(`unknown option: ${argv[index]}\n`); return { exitCode: 4 }; } }
