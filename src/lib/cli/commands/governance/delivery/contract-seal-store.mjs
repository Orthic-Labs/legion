...
import { validateExecutableContract, collectExecutableContractErrors } from '../../../../../packages/contracts/executable.mjs';
...
export class ContractSealStore {
  ...
  seal({ contract, authorityAssertion, dispatchDigest = null, evidenceReachability = null }) {
    ...
    const errors = collectExecutableContractErrors(contract); if (errors.length) fail(errors.some((e) => e.code.startsWith('EXEC_')) ? 'ARC_CONTRACT_NOT_EXECUTABLE' : 'ARC_SCHEMA_INVALID', errors[0].message);
    try { validateExecutableContract(contract); } catch { fail('ARC_CONTRACT_NOT_EXECUTABLE', 'contract is not executable'); }
    if (!authorityAssertion || !['legion', 'sage'].includes(authorityAssertion.authority) || typeof authorityAssertion.assertedBy !== 'string' || !authorityAssertion.assertedBy || authorityAssertion.verificationMethod !== 'capability-signature' || authorityAssertion.perMessage !== true) fail('ARC_SCHEMA_INVALID', 'invalid contract-seal authority assertion (legion | sage)');
    const record = { schemaVersion: 1, kind: 'arcane-contract-seal', contractId: contract.contractId, version: contract.version, sourceRevision: contract.sourceRevision, contractDigest: digestValue(contract), dispatchDigest, sealedBy: { authority: authorityAssertion.authority, assertedBy: authorityAssertion.assertedBy, verificationMethod: 'capability-signature', perMessage: true }, sealedAt: this.#clock(), contract };
