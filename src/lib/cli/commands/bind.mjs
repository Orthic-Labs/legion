...
import * as gemini from './bind/gemini.mjs';
import * as agentsMd from './bind/agents-md.mjs';
import { artifactForTarget, bindingReceiptPath, driftForHarness, readBindingReceipt } from './bind/drift.mjs';
import { enumerateRegistrations } from './bind/registrations.mjs';

const HARNESS_NAMES = ['claude-code', 'codex', 'gemini', 'agents-md'];
const HARNESS_MODULES = { 'claude-code': claudeCode, codex, gemini, 'agents-md': agentsMd };
...
export function runBind(argv, { stdout, stderr, cwd }) {
  ...
  const harnesses = targetNames.map((name) => {
    ...
    const targets = mod.targets(root);
    const wouldWrite = targets.map(({ path, reason }) => ({ path, reason }));
    const priorEntry = priorByName.get(name);
    const drift = priorEntry ? driftForHarness(root, priorEntry) : [];
    const managedConflicts = targets
      .filter((target) => target.kind === 'toml-managed' && target.result.status === 'managed_conflict')
      .map((target) => ({ path: target.path, conflicts: target.result.conflicts }));
    return {
      name,
  ...
  let hasDrift = harnesses.some((h) => h.drift.length > 0 || h.managedConflicts.length > 0);
  ...
  if (!dryRun) {
    ...
    let hasManagedConflict = false;
    for (const entry of report.harnesses) {
      const mod = HARNESS_MODULES[entry.name];
      const { wrote, managedConflicts = [] } = mod.write(root);
      entry.wrote = wrote;
      entry.managedConflicts = managedConflicts;
      hasManagedConflict ||= managedConflicts.length > 0;
      receiptHarnesses.push({ name: entry.name, fidelityTier: entry.fidelityTier, files: entry.artifacts });
    }
    if (!hasManagedConflict) {
      const receipt = {
        schemaVersion: 2,
