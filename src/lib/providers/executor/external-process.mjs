...
const boundedProbeTimeout=(value)=>{const timeout=Number(value??5000);return Number.isFinite(timeout)&&timeout>=1?Math.min(Math.floor(timeout),5000):5000;};
...
export async function runExternal(spec,host={}){
  ...
  const tool=await qualifyTool({executable:spec.executable,versionArgs:spec.versionArgs??['--version'],cwd:spec.cwd,env:environment,timeoutMs:boundedProbeTimeout(spec.versionTimeoutMs)});
