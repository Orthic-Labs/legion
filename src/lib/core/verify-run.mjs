...
export async function verifySealedRun({ priorRun, currentRepository }, host) {
  ...
  return { schemaVersion: 1, kind: 'legion-verification-receipt', valid, priorDigest: priorSemantic,currentDigest:currentSemantic, binding: currentBinding, status: valid ? 'pass' : 'unproven', gaps, verifiedAt: host.clock.now().toISOString() };
}
