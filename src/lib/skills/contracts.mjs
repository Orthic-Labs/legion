...
export function validateSkillBundle(bundle) {
  ...
  const paths=new Set(),uris=new Set();for(const file of bundle.files??[]){if(!file.path||file.path.startsWith('/')||file.path.split('/').includes('..'))throw new TypeError('skill file outside root');if(paths.has(file.path))throw new TypeError(`duplicate skill file: ${file.path}`);paths.add(file.path);const match=URI.exec(file.uri??'');if(!match||match[1]!==bundle.id||match[2]!==file.path)throw new TypeError(`skill URI must match bundle path: ${file.uri}`);if(uris.has(file.uri))throw new TypeError(`duplicate skill URI: ${file.uri}`);uris.add(file.uri);if(!SHA.test(file.digest??''))throw new TypeError(`skill digest missing: ${file.path}`);}
