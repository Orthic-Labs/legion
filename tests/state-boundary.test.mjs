...
// Pins the HeardRight escape: a test wrote a fake route into the app's real
...
function scenario() {
  ...
  const appData = join(root, 'HeardRightNext');
...
test('the HeardRight contamination is caught: a test rewrites production state', async () => {
  const s = scenario();
  try {
    await runState(['snapshot', '--path', s.appData, '--out', s.snap], s.ctx);
