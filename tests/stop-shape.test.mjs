...
test('a genuine Adrian-only next action still passes', () => {
  // Actions only Adrian can perform must remain a legal ending.
  for (const ending of [
...
test('deferring a found defect blocks', () => {
  for (const ending of [
    "Worth flagging for later: viewright's committed AGENTS.md is still stale doctrine on origin. That's a separate cleanup whenever you want it.",
...
test('legitimate deferral passes', () => {
  for (const ending of [
    ...
    "Worth flagging: viewright's AGENTS.md is stale — filed as a background task via spawn_task.",
    'Deploying the binary is a separate cleanup, and it is ADRIAN-ONLY per HANDOFF.',
  ]) {
    assert.equal(evaluateStopShape(ending).block, false, ending);
  }
});
...
test('the override tag is honoured', () => {
  const text = 'Worth flagging for later: this is a separate cleanup. [deferred-ok: Adrian owns this call]';
  assert.equal(evaluateStopShape(text).block, false);
});
...
test('an invented reserved category does not launder a stop-short', () => {
  ...
  const v = evaluateStopShape('BLOCKED-ON-APPROVAL: installing ~/.claude/bin/rhook.exe for the fast-path twin — reserved to Adrian per HANDOFF.');
  assert.equal(v.block, true);
...
// and explicitDirective(). Only fires when Adrian's own recent words carried
