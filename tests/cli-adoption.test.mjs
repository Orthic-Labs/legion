...
test('production manifest → atomic admission → twelve authenticated status reads', async () => {
  ...
  try {
    ...
    const manifest = { schemaVersion: 1, kind: 'legion-adoption-oracle-manifest', runId, taskId: 'T-1', contractId: contract.contractId, contractVersion: contract.version, contractDigest: sealed.contractDigest, sourceRevision: contract.sourceRevision, acceptanceFingerprint: ledger.acceptance_fingerprint, integratedState, observedAt: eventTime, items: stageIds.map((stageId, index) => ({ acceptanceId: formalCliItemId(stageId), verdict: 'PASS', evidenceId: `sha256:${String(index + 1).padStart(64, '0')}`, requirementId: `R-${index + 1}`, productionSymbol: `symbol-${index + 1}`, liveConsumer: 'legion adoption status', acceptanceSurface: 'node --test', observedAt: eventTime })) };
