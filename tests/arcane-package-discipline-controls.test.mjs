...
import { auditSuccessfulCommit, commitReceiptRequirement, commitRepository, generatedLockTargets, preEffectDiscipline } from '../src/lib/host/arcane/discipline-controls.mjs';
...
test('nested repository paths are framed from workspace root before locked-domain matching', () => {
  ...
  // `legion/src/lib/guard/**`, so `legion` is a direct child of the
  ...
  try {
    mkdirSync(join(repository, 'src', 'lib', 'guard'), { recursive: true });
    ...
    writeFileSync(join(repository, 'src', 'lib', 'guard', 'gate.mjs'), 'export const gate = true;\n');
    execFileSync('git', ['add', 'src/lib/guard/gate.mjs'], { cwd: repository, windowsHide: true });
    const policy = { lockedDomainsFor: (paths) => paths.filter((path) => path.startsWith('legion/src/lib/guard/')).map((path) => ({ path })) };
    const requirement = commitReceiptRequirement({ workspace, repository, policy });
    assert.equal(requirement.required, true);
    assert.deepEqual(requirement.paths, ['legion/src/lib/guard/gate.mjs']);
