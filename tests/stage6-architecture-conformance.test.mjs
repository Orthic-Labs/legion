...
test('S06-01 exact workflow manifest & machine-readable phase contracts', async () => {
  const index = await read('skills/architect/doctrine/architecture/README.md');
  assert.deepEqual([...index.matchAll(/workflow\/(\d\d-[a-z-]+)\.md/g)].map((m) => m[1]), manifest);
  for (const [ordinal, name] of manifest.entries()) {
    const contract = tagged(await read(`skills/architect/doctrine/architecture/workflow/${name}.md`), 'architecture-workflow-module.v1');
...
test('S06-01 gates, current-only resume, reopen & minimization sequence', async () => {
  ...
  const all = await Promise.all(manifest.map((name) => read(`skills/architect/doctrine/architecture/workflow/${name}.md`)));
  const text = all.join('\n');
  ...
  const evaluate = await read('skills/architect/doctrine/architecture/workflow/07-evaluate-select.md');
...
test('S06-01 method contracts are conditional & reject prohibited automatic use', async () => {
  ...
  for (const name of methods) {
    const contract = tagged(await read(`skills/architect/doctrine/architecture/methods/${name}.md`), 'architecture-method-module.v1');
  ...
  const text = await Promise.all(methods.map((name) => read(`skills/architect/doctrine/architecture/methods/${name}.md`))).then((all) => all.join('\n'));
...
test('S06-01 Architect owns architecture method; no Sage bundle route remains active', async () => {
  ...
  const readme = await read('skills/architect/doctrine/architecture/README.md');
