...
ROOT = HERE.parent
SCRIPT = ROOT / "lib" / "handoff" / "transcript-handoff.py"
MODULE = ROOT / "lib" / "handoff" / "transcript_handoff.py"


def load_module():
    spec = importlib.util.spec_from_file_location("transcript_handoff", MODULE)
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="handoff-transcript-") as directory:
        ...
        mac_prompt = module.paste_prompt({
            **pointer,
            "platform": "claude",
            "workspace": "/Volumes/D/claude",
            "source_path": "/Users/test/.claude/projects/p/claude-test.jsonl",
        })
        assert mac_prompt.startswith("You are target chat")
        assert 'python3 "/Volumes/D/claude/tools/skills/legion/lib/handoff/transcript-handoff.py"' in mac_prompt
