...
import {
  ...
  reverseEnvelope,
  FIXTURES_DIR,
} from '../src/lib/host/arcane/legacy-bridge.mjs';
import { validateAgainst } from '../src/lib/contracts/arcane/validate.mjs';
import { canonicalJson, digestValue } from '../src/lib/contracts/arcane/canonical.mjs';
...
test('S01: bridging a real S00 store-snapshot fixture yields valid envelopes with authenticated:false', () => {
  ...
  for (const env of result.envelopes) {
    ...
    assert.equal(
      ...
      'src/lib/host/arcane-compatibility/forge/schema-map.json',
    );
  }
});
...
test('S01: the bridge contains no independent policy decision (alias rule)', () => {
  const src = readFileSync(path.join(__dirname, '..', 'src', 'lib', 'host', 'arcane', 'legacy-bridge.mjs'), 'utf8');
