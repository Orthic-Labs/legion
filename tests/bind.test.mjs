...
test('codex bind refreshes its stale generated current-role block without touching user config', () => {
  const dir = mkdtempSync(join(tmpdir(), 'legion-codex-generated-bind-'));
  try {
    mkdirSync(join(dir, '.codex'), { recursive: true });
    writeFileSync(join(dir, '.codex', 'config.toml'), [
      'model = "gpt"', '', '# Generated by Legion bind.',
      '# Owned keys: [agents.sage], [agents.alchemist], [agents.oracle], [agents.covenant-seat], [mcp_servers.legion].',
      '[mcp_servers.legion]', 'command = "node"', 'args = ["old.mjs"]', '',
      '[agents.sage]', 'description = "old"', 'config_file = "agents/sage.toml"', '',
      '[agents.alchemist]', 'description = "old"', 'config_file = "agents/alchemist.toml"', '',
      '[agents.oracle]', 'description = "old"', 'config_file = "agents/oracle.toml"', '',
      '[agents.covenant-seat]', 'description = "old"', 'config_file = "agents/covenant-seat.toml"', '',
      '[history]', 'persistence = "save-all"', '',
    ].join('\n'));
    const written = bind(['--write', '--harness', 'codex', dir]);
    assert.equal(written.status, 0, written.stderr);
