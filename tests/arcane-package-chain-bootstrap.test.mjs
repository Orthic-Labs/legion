...
import { workspaceRelative } from '../src/lib/guard/compat/effects/preeffect-gate.mjs';
...
test('an absolute host path is matched against a relative scope entry', () => {
  assert.equal(
    workspaceRelative('D:/workspace/legion/src/lib/cognitive/arcane/stop-shape.mjs', 'D:/workspace'),
    'legion/src/lib/cognitive/arcane/stop-shape.mjs',
  );
});
...
test('a path outside the workspace is left absolute, so it still fails own[]', () => {
  const outside = workspaceRelative('C:/Windows/System32/drivers/etc/hosts', 'D:/workspace');
...
test('a sibling directory sharing the workspace prefix is not treated as inside it', () => {
  // 'D:/workspace-backup' must not relativize against 'D:/workspace'.
  assert.equal(workspaceRelative('D:/workspace-backup/secret.txt', 'D:/workspace'), 'D:/workspace-backup/secret.txt');
});

test('an already-relative path passes through unchanged', () => {
  assert.equal(workspaceRelative('tools/a.mjs', 'D:/workspace'), 'tools/a.mjs');
});

test('with no workspace known, the path is left alone rather than guessed at', () => {
  assert.equal(workspaceRelative('D:/workspace/tools/a.mjs', null), 'D:/workspace/tools/a.mjs');
});
...
test('maxLength is enforced, so the schema caps are a gate and not a comment', async () => {
  const { RuntimeSchemaSet } = await import('../src/lib/contracts/arcane/runtime-schema.mjs');
