...
test('formal adoption consumes one authenticated exact-item Oracle manifest & admits complete DAG atomically', () => {
  ...
  const manifest = { schemaVersion: 1, kind: 'legion-adoption-oracle-manifest', ...execution, acceptanceFingerprint: ledger.acceptance_fingerprint, integratedState, observedAt: '2026-08-14T12:00:00Z', items: stageIds.map((stageId, index) => { const evidence = { kind: 'oracle-adoption-item-evidence', sources: [{ path: `source-${index + 1}`, digest: `sha256:${String(index + 1).padStart(64, '0')}` }], checks: ['structured production inspection'] }; return { acceptanceId: formalAdoptionItemId(stageId), verdict: 'PASS', evidence, evidenceId: digestValue(evidence), requirementId: `R-${index + 1}`, productionSymbol: `symbol-${index + 1}`, liveConsumer: 'legion adoption status', acceptanceSurface: 'node --test', observedAt: '2026-08-14T11:00:00Z' }; }) };
