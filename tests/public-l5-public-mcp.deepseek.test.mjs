...
function fixtureRoot() {
  const root = mkdtempSync(join(tmpdir(), 'nemesis-deepseek-'));
...
test('B8-003 canonical core dispatch: doctor, plan, verify, and read tools execute in-process', async () => {
  ...
  assert.deepEqual(listTools().map(({ name }) => name), [
    'nemesis_doctor', 'nemesis_plan', 'nemesis_audit', 'nemesis_verify',
    'nemesis_get_run', 'nemesis_get_finding', 'nemesis_explain',
    'nemesis_list_providers', 'nemesis_list_languages', 'nemesis_list_families', 'nemesis_list_skills',
  ]);
  assert.ok(listTools().every(({ inputSchema }) => inputSchema.additionalProperties === false));

  const doctor = await callTool({ name: 'nemesis_doctor', arguments: { root: '.' } }, { root, maxOutputBytes: 4_000_000 });
  assert.equal(doctor.isError, false);
  assert.equal(doctor.structuredContent.kind, 'nemesis-product-inspection');

  const plan = await callTool({ name: 'nemesis_plan', arguments: { root: '.', profile: 'fast' } }, { root, maxOutputBytes: 4_000_000 });
  assert.equal(plan.isError, false);
  assert.equal(plan.structuredContent.plan.kind, 'nemesis-sealed-plan');
  assert.match(plan.structuredContent.plan.planDigest, /^sha256:/);

  const verify = await callTool({ name: 'nemesis_verify', arguments: { root: '.', priorRun: join('.audit', 'run-1', 'report.json') } }, { root, maxOutputBytes: 4_000_000 });
  assert.equal(verify.isError, false);
  assert.equal(verify.structuredContent.kind, 'nemesis-verification-receipt');
  assert.equal(verify.structuredContent.valid, false);
  assert.ok(verify.structuredContent.gaps.some(({ kind }) => kind === 'binding-drift'));

  const run = await callTool({ name: 'nemesis_get_run', arguments: { run: '.audit/run-1', artifact: 'report.json' } }, { root });
  ...
  const finding = await callTool({ name: 'nemesis_get_finding', arguments: { run: '.audit/run-1', findingId: 'f-1' } }, { root });
  assert.equal(finding.isError, false);
  assert.equal(finding.structuredContent.finding.ruleId, 'xss');

  const explain = await callTool({ name: 'nemesis_explain', arguments: { run: '.audit/run-1', id: 'f-1' } }, { root });
  assert.equal(explain.isError, false);
  assert.equal(explain.structuredContent.kind, 'nemesis-explain');
...
test('B8-003 canonical core dispatch: list tools read the sealed registry in-process', async () => {
  ...
  const root = fixtureRoot();
  const providers = await callTool({ name: 'nemesis_list_providers', arguments: {} }, { root });
  ...
  const families = await callTool({ name: 'nemesis_list_families', arguments: {} }, { root });
  assert.equal(families.isError, false);
  assert.ok(families.structuredContent.families.includes('framework.react'));
  for (const name of ['nemesis_list_languages', 'nemesis_list_skills']) {
    const result = await callTool({ name, arguments: {} }, { root });
    assert.equal(result.isError, false);
    assert.ok(Array.isArray(result.structuredContent[name.slice('nemesis_list_'.length)]));
  }
});
...
test('B8-003 rejects scope escape and bounded outputs on canonical dispatch', async () => {
  ...
  for (const args of [{ root: '..' }, { root: `..${sep}other` }]) {
    assert.equal((await callTool({ name: 'nemesis_doctor', arguments: args }, { root })).isError, true);
  }
  assert.equal((await callTool({ name: 'nemesis_verify', arguments: { root: '.', priorRun: join('..', 'report.json') } }, { root })).isError, true);
  assert.equal((await callTool({ name: 'nemesis_get_run', arguments: { run: '..' } }, { root })).isError, true);
  const oversized = await callTool({ name: 'nemesis_plan', arguments: { root: '.' } }, { root, maxOutputBytes: 16 });
...
test('resources expose the canonical run report with bounded, escape-safe reads', async () => {
  ...
  assert.equal(list[0].uri, 'nemesis://run/report');
  assert.equal(list[0].mimeType, 'application/json');
  assert.throws(() => readResource({ uri: 'nemesis://other', run: runDir }, { root }), /unknown resource/);
  assert.throws(() => readResource({ uri: 'nemesis://run/report', run: '..' }, { root }), /escapes host root/);
  const read = readResource({ uri: 'nemesis://run/report', run: runDir }, { root });
  assert.equal(read.contents[0].uri, 'nemesis://run/report');
  assert.equal(JSON.parse(read.contents[0].text).findings[0].id, 'f-1');
  assert.throws(() => readResource({ uri: 'nemesis://run/report', run: runDir }, { root, maxBytes: 8 }), /exceeds configured limit/);
});
