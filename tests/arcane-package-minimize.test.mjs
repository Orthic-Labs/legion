...
import {
  ...
  verifyReceipt,
  writeJson,
} from '../src/lib/cognitive/arcane/minimize.mjs';

const POLICY = new URL('../src/lib/cognitive/arcane/policy/minimize-policy.md', import.meta.url).pathname.replace(/^\/([A-Za-z]:)/, '$1');
const VALIDATOR = new URL('../src/lib/cognitive/arcane/minimize.mjs', import.meta.url).pathname.replace(/^\/([A-Za-z]:)/, '$1');
