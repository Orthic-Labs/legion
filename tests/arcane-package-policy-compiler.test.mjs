...
import { compilePolicyRules, parsePolicyRules } from '../src/lib/guard/compat/rules/policy-compiler.mjs';

const root = path.dirname(fileURLToPath(new URL('../src/lib/guard/compat/policy/arcane-policy-v1.json', import.meta.url)));
const base = JSON.parse(readFileSync(path.join(root, 'arcane-policy-v1.json'), 'utf8'));
const source = readFileSync(fileURLToPath(new URL('../src/lib/guard/compat/rules/arcane-policy-v1.rules', import.meta.url)), 'utf8');
