...
} from '../src/lib/guard/compat/policy/policy.mjs';
import { AuthorityLedger, extractAuthorityFromPayload, requireAuthority } from '../src/lib/contracts/arcane/authority.mjs';
import { EFFECT_CLASS, AUTHENTICATION_METHOD } from '../src/packages/contracts/index.mjs';
import { ArcaneError } from '../src/lib/contracts/arcane/errors.mjs';

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const repositoryRoot = path.join(__dirname, '..');
const bridgePath = path.join(repositoryRoot, 'src', 'lib', 'host', 'arcane', 'legacy-bridge.mjs');
const evidenceEnvelopePath = path.join(repositoryRoot, 'src', 'lib', 'verification', 'arcane', 'evidence-envelope.mjs');
const arcaneRuntimeRoots = [
  path.join(repositoryRoot, 'src', 'lib', 'cognitive', 'arcane'),
  path.join(repositoryRoot, 'src', 'lib', 'contracts', 'arcane'),
  path.join(repositoryRoot, 'src', 'lib', 'guard', 'compat'),
  path.join(repositoryRoot, 'src', 'lib', 'host', 'arcane'),
  path.join(repositoryRoot, 'src', 'lib', 'host', 'arcane-compatibility'),
  path.join(repositoryRoot, 'src', 'lib', 'verification', 'arcane'),
];
...
test('S02: no module outside preeffect-gate.mjs / receipt-store.mjs mints a capability', () => {
  const files = arcaneRuntimeRoots.flatMap(moduleFiles);
