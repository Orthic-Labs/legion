...
test('B1-003 command catalogue generates complete strict CLI help', async () => {
  ...
  assert.deepEqual(COMMANDS.map(({ name }) => name), ['init','doctor','inspect','targets','components','stacks','controls','skills','languages','providers','rules','schedule','plan','audit','verify','explain','report','fix','hooks','mcp']);
...
test('B1-024 skill transformation is deterministic, neutral, and strips audit mutation authority', async () => {
  ...
  const rules = [{ from: 'Adrian', to: 'the approving human' }];
