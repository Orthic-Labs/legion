...
test('B1-002 canonical enums reject unknown values and future schema versions', async () => {
  const { PROVIDER_STATUS, assertEnum, assertSchemaVersion } = await import('../../src/lib/contracts/enums.mjs');
...
test('B1-004 fixed host returns typed unavailable capability receipts', async () => {
  const { fixedHost } = await import('../../src/lib/host/fixed-host.mjs');
  const { requireCapability } = await import('../../src/lib/host/capabilities.mjs');
...
test('B1-005 artifact paths reject alternate separators and digests bind bytes', async () => {
  const { safeArtifactPath } = await import('../../src/lib/artifacts/paths.mjs');
  const { digestBytes } = await import('../../src/lib/artifacts/digests.mjs');
...
test('B1-006 run manifest rejects orphan and missing declared files', async () => {
  const { validateRunManifest } = await import('../../src/lib/core/run-manifest.mjs');
...
test('B1-007 trusted merge rejects repository attempts to grant host policy', async () => {
  const { mergeTrustedConfig } = await import('../../src/lib/config/merge.mjs');
...
test('B1-008 sealed profiles expose resource, cache, cancellation, schedule, and reasoning policy', async () => {
  const { sealedProfile } = await import('../../src/lib/config/profiles.mjs');
...
test('B1-009 family and lens registries reject dependency cycles', async () => {
  const { validateFamilies } = await import('../../src/lib/contracts/families.mjs');
  const { validateLenses } = await import('../../src/lib/contracts/lenses.mjs');
...
test('B1-010 provider v2 rejects candidate generators that emit findings', async () => {
  const { validateProviderV2, validateProviderOutputAuthority } = await import('../../src/lib/providers/sdk/contracts.mjs');
...
test('B1-011 schedule compiler uses lexical waves and rejects impossible resources', async () => {
  const { compileSchedule } = await import('../../src/lib/providers/sdk/schedule.mjs');
...
test('B1-012 reasoning runner prepares a packet without invoking a reviewer', async () => {
  const { executeRunner } = await import('../../src/lib/providers/executor/index.mjs');
  let invoked = false;
...
test('B1-014 binding comparison is stable across path separators', async () => {
  const { sameBinding } = await import('../../src/lib/core/binding.mjs');
...
test('B1-016 scheduler preserves independent work when one dependency blocks', async () => {
  const { scheduleProviders } = await import('../../src/lib/core/scheduler.mjs');
...
test('B1-017 zero findings with a required gap is incomplete', async () => {
  const { reconcileClaim } = await import('../../src/lib/core/completeness.mjs');
...
test('B1-018 reviewer policy rejects context reuse and self-adjudication', async () => {
  const { reviewerPolicy } = await import('../../src/lib/core/reviewer-policy.mjs');
...
test('B1-019 report model preserves topology, controls, and per-claim decisions without a score', async () => {
  const { buildReportModel } = await import('../../src/lib/report/model.mjs');
...
test('B1-020 semantic verification excludes time, paths, and presentation ordering', async () => {
  const { semanticProjection } = await import('../../src/lib/verification/projection.mjs');
...
test('B1-021 schedule command is a pure canonical renderer', async () => {
  const { renderSchedule } = await import('../../src/lib/cli/commands/schedule.mjs');
...
test('B1-022 skill bundle contract rejects absolute internal URIs and separates profiles', async () => {
  const { validateSkillBundle } = await import('../../src/lib/skills/contracts.mjs');
...
test('B1-023 skill URI resolver and verifier are package-relative and digest-bound', async () => {
  const { parseSkillUri } = await import('../../src/lib/skills/uri.mjs');
  const { verifySkillBytes } = await import('../../src/lib/skills/verify.mjs');
