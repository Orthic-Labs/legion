...
test('all 135 legacy paths remain in standalone Legion', () => {
  ...
  // Re-baselined 2026-08-14 (104/31 -> 94/41): public entrypoint packaging, semantic aliases,
  // import-safe script guards, and package-local receipt binding intentionally changed ten mapped files. Every path remains;
  // only byte parity moved.
  assert.equal(records.filter((record) => record.currentBlob === record.legacyBlob).length, 94);
  assert.equal(records.filter((record) => record.currentBlob !== record.legacyBlob).length, 41);
});
