...
export function verifyWebAccessibility(input = {}) {
  if (!Array.isArray(input.captures) || !Array.isArray(input.inspections) || input.captures.some((item) => !item || typeof item !== 'object') || input.inspections.some((item) => !item || typeof item !== 'object')) return finalize('legion-web-accessibility-evidence', { provider: 'runtime.web.accessibility', status: 'error', terminal: true, binding: input.binding ?? {}, denominator: denominator([], []), inspections: [], violationCount: 0, coverageGaps: ['accessibility-collections-invalid'] });
