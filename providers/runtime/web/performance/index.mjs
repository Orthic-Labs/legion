...
export function verifyWebPerformance(input = {}) {
  if (!Array.isArray(input.captures) || input.captures.some((item) => !item || typeof item !== 'object' || Array.isArray(item))) return finalize('nemesis-web-performance-evidence', { provider: 'runtime.web.performance', status: 'error', terminal: true, claimLevel: 'runtime', evidenceClass: 'measured', binding: input.binding ?? {}, metrics: { lcp: null, longTasks: null, layoutShifts: null }, budgets: input.budgets ?? {}, captureDigest: null, artifacts: [], coverageGaps: ['performance-captures-invalid'] });
  ...
  return finalize('nemesis-web-performance-evidence', { provider: 'runtime.web.performance', status: gaps.some((gap) => gap.startsWith('budget-exceeded:')) ? 'fail' : gaps.length ? 'unproven' : 'pass', terminal: true, claimLevel: 'runtime', evidenceClass: 'measured', binding: input.binding ?? {}, metrics, budgets: input.budgets ?? {}, captureDigest: capture.digest, artifacts: capture.captures, coverageGaps: [...new Set(gaps)].sort() });
}
