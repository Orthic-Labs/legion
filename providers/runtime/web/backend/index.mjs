...
export function inspectWebBackend({ binding = {}, endpoints = [] } = {}) {
  if (!Array.isArray(endpoints) || endpoints.some((item) => !item || typeof item !== 'object')) return finalize('legion-web-backend-inventory', { status: 'error', terminal: true, binding, denominator: denominator([], []), receipts: [], coverageGaps: ['backend-collection-invalid'] });
  ...
  return finalize('legion-web-backend-inventory', { status: gaps.length ? 'unproven' : 'pass', terminal: true, claimLevel: 'source', binding, denominator: counts, receipts, coverageGaps: gaps.sort() });
}
