...
export function discoverWebSurfaces({ artifacts = [], targetId, componentId } = {}) {
  ...
  return { schemaVersion: 1, kind: 'nemesis-web-surface-inventory', targetId, surfaces, complete: surfaces.every((item) => item.componentId), coverageGaps: surfaces.filter((item) => !item.componentId).map((item) => `component-unbound:${item.id}`) };
}
