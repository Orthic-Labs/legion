...
named `/council` workflow only when Adrian explicitly asks for Council, a jury, or external review.
...
- **External review is explicit opt-in for every design.** Apply the risk/blast-radius definition
  above and complete inline evidence review. Run `/council` or `runAutoJury` only when Adrian names
