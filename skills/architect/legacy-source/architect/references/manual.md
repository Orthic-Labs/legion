...
design, challenge the ADR against code/tests/logs, then proceed without waiting for the operator. Run the
named `/council` workflow only when the operator explicitly asks for Council, a jury, or external review.
...
These are same-context perspectives, not independent reviewers and not the named `/council` skill.
Their job is cheap option improvement before the ADR. Flag real divergence instead of forcing
agreement, but do not spawn one agent per role by default. When the operator explicitly opts into
`/council`, its advisory, revision, and Jury verdict run later. For unusually wide or irreversible
...
lenses and evidence checks. `/council` is an explicit opt-in external review, never an automatic
...
End with the implementation handle (SampleApp contract):
```
...
challenges claims against evidence. It does not claim independence. External `/council` review is
...
- **External review is explicit opt-in for every design.** Apply the risk/blast-radius definition
  above and complete inline evidence review. Run `/council` or `runAutoJury` only when the operator names
  that mechanism; ordinary substantive work proceeds without an external gate.
- When requested, Council advises and its internal Jury reports a verdict; neither wins merely
  because of its label. Resolve conflicting claims with code, tests, logs, or a targeted rerun. the operator
...
instead of dumping outputs, integrates, then verifies the combined result. Ask the operator only when the
