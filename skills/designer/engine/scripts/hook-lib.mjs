...
export const GENERATED_PATH = /(?:\.generated\.[a-z]+$|\.d\.ts$|\.min\.[a-z]+$|[/\\]node_modules[/\\]|[/\\](nemesis-skill://designer/engine/scripts/?:dist|build|out|/.next|/.cache|coverage)[/\\]|[/\\]?[^/\\]+\.lock(?:\.json)?$)/i;
...
export function suppressionNotice(filePath) {
  return `${ENVELOPE_PREFIX} Suppressing further design hints on ${filePath}. More than ${EDIT_COUNT_THRESHOLD} edits in this session reached. Run /designer audit to revisit.`;
}
...
export function renderTemplate(findings, filePath, config, opts = {}) {
  ...
  const more = remaining > 0
    ? `... and ${remaining} more (see /designer audit).`
    : null;
...
function renderGroupedTemplate(groups, config, opts = {}) {
  ...
  for (const group of realGroups) {
    ...
    if (hidden > 0) {
      lines.push(`- ... ${hidden} more in ${display} (see /designer audit).`);
    }
  }
...
function clampGroupedToBudget(header, lines, footer, maxChars) {
  const assemble = (linesArr, omitted) => [
    header,
    ...linesArr,
    ...(omitted ? ['... and more (see /designer audit).'] : []),
    '',
    footer,
  ].join('\n');
...
function clampToBudget(header, lines, more, footer, maxChars) {
  ...
  while (assembled.length > maxChars && working.length > 1) {
    working.pop();
    moreText = '... and more (see /designer audit).';
...
function formatFindingIgnoreCommand(finding) {
  ...
  return `/designer hooks ignore-value ${rule} ${valueArg} --shared --reason ${reason}`;
}
...
const STATIC_STYLE_IMPORT_RE = /import\s+(?:[\w*{}\s,$]+\s+from\s+)?['"](nemesis-skill://designer/engine/scripts/[^'"]+/.(?:css|scss|sass|less))['"]/gi;
...
function directiveFooter(display, opts = {}) {
  const ignoreFileCommand = `/designer hooks ignore-file ${quoteCommandArg(display)}`;
  const fileIgnoreGuidance = opts.grouped
    ? 'run `/designer hooks ignore-file <path>` for the specific file'
