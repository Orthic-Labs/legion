#!/usr/bin/env node
/**
 * `/designer hooks <on|off|status|reset>` — manage the design hook runtime
...
const HOOK_MANIFEST_TARGETS = [
  {
    provider: '.claude',
    skillRel: '.claude/skills/designer',
  ...
  {
    provider: '.agents',
    skillRel: '.agents/skills/designer',
  ...
  {
    provider: '.cursor',
    skillRel: '.cursor/skills/designer',
  ...
  },
  {
    ...
    provider: '.github',
    skillRel: '.github/skills/designer',
    destRel: '.github/hooks/designer.json',
    manifest: () => ({
      version: 1,
      hooks: {
        postToolUse: [
...
function writeHookConfig(cwd, hookConfig, opts = {}) {
  ...
  // (consent, quiet, auditLog) survive a `/designer hooks` edit.
...
function addIgnoreRule(cwd, args) {
  ...
  if (!rule) throw new Error('Pass a rule id, e.g. /designer hooks ignore-rule side-tab');
  if (rule === 'overused-font' && !parsed.allValues) {
    throw new Error('overused-font is value-specific by default. Use /designer hooks ignore-value overused-font <font> for a confirmed font, or /designer hooks ignore-rule overused-font --all-values only when the user asked to ignore overused fonts generally.');
  }
...
function addIgnoreFile(cwd, glob) {
  if (!glob) throw new Error('Pass a glob, e.g. /designer hooks ignore-file "src/legacy/**"');
  const config = mergeDetectorConfig(readRawDetectorConfig(cwd));
...
function addIgnoreValue(cwd, args) {
  ...
  if (!parsed.rule || !parsed.value) {
    throw new Error('Pass a rule id and value, e.g. /designer hooks ignore-value overused-font Inter');
  }
