...
async function detectCli() {
  ...
  // --viewport=WxH / --mobile control the rendered viewport for URL scans.
  // Structure rules (fold position, hero geometry) depend on it; run websites
  // at BOTH the default desktop viewport and --mobile.
  const viewportArg = args.find(a => a.startsWith('--viewport='));
  if (viewportArg) {
    const m = viewportArg.match(/^--viewport=(\d+)x(\d+)$/);
    if (!m) { process.stderr.write('Error: --viewport expects WxH, e.g. --viewport=1440x900\n'); process.exit(1); }
    scanOptions.viewport = { width: Number(m[1]), height: Number(m[2]) };
  } else if (args.includes('--mobile')) {
    scanOptions.viewport = { width: 390, height: 844 };
  } else if (args.includes('--tablet')) {
    scanOptions.viewport = { width: 768, height: 1024 };
  }
  const siteSweep = args.includes('--site');
  const siteTypeArg = args.find(a => a.startsWith('--site-type='));
  const siteType = siteTypeArg ? siteTypeArg.split('=')[1] : null;
  if (siteType && !['app', 'ecommerce', 'content'].includes(siteType)) {
    process.stderr.write('Error: --site-type must be app, ecommerce, or content\n');
    process.exit(1);
  }
      ...
      for (const target of paths) {
        if (/^https?:\/\//i.test(target)) {
          try {
            ...
            if (siteSweep) {
              const { sweepSite } = await import('../engines/site/sweep.mjs');
              allFindings.push(...await sweepSite(target, { siteType }));
            }
