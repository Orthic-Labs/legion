...
const ANTIPATTERNS = [
  ...
  // ── Website structure: conversion geometry, measured on the rendered page ──
  {
    id: 'cta-below-fold',
    category: 'structure',
    name: 'Primary CTA below the fold',
    description:
    ...
    skillSection: 'Structure',
    skillGuideline: 'primary CTA above the fold',
  },
  {
    id: 'hero-cta-competition',
    category: 'structure',
    name: 'Competing CTAs above the fold',
    description:
    ...
    skillSection: 'Structure',
    skillGuideline: 'one primary CTA decision',
  },
  {
    id: 'headline-word-wall',
    category: 'structure',
    name: 'Headline word wall',
    description:
    ...
    skillSection: 'Structure',
    skillGuideline: 'descriptive headline under ten words',
  },
  {
    id: 'one-word-lines',
    category: 'structure',
    name: 'Heading wraps to word-per-line column',
    description:
    ...
    skillSection: 'Structure',
    skillGuideline: 'headline wraps as a readable line, not a word stack',
  },
  {
    id: 'missing-hero-media',
    category: 'structure',
    severity: 'advisory',
    name: 'No visual anchor above the fold',
    description:
    ...
    skillSection: 'Structure',
    skillGuideline: 'hero imagery shows the product',
  },
  {
    id: 'hero-viewport-hog',
    category: 'structure',
    name: 'Text-wall hero over a full viewport',
    description:
    ...
    skillSection: 'Structure',
    skillGuideline: 'hero states the offer inside one viewport',
  },
  {
    id: 'hover-contrast',
    category: 'structure',
    name: 'Illegible hover state',
    description:
    ...
    skillSection: 'Color & Contrast',
    skillGuideline: 'hover states stay legible',
  },
  {
    id: 'oversized-header',
    category: 'structure',
    name: 'Oversized header or wordmark',
    description:
    ...
    skillSection: 'Structure',
    skillGuideline: 'header and wordmark scale',
  },
];
...
if (IS_BROWSER) {
  ...
  // ─── Website structure / conversion geometry (page-level, browser only) ────
  // Measured rules for the failures prose lenses keep missing: CTA pushed
  // below the fold, word-stack headlines, text-wall heroes, no visual anchor.
  // Only judges landing-shaped pages (h1 near the top); app shells without an
  // above-the-fold h1 are left alone.
  function checkWebsiteStructureDOM() {
    const findings = [];
    const win = window;
    const doc = document;
    const vh = win.innerHeight;
    const vw = win.innerWidth;
    if (!vh || !vw || !doc.body) return findings;
    const scrollY = win.scrollY || 0;

    const h1 = doc.querySelector('h1');
    if (!h1) return findings;
    const h1Rect = h1.getBoundingClientRect();
    const h1Top = h1Rect.top + scrollY;
    // Not landing-shaped: h1 starts beyond ~1.5 viewports (article body, app view).
    if (h1Top > vh * 1.5 || h1Rect.width < 2 || h1Rect.height < 2) return findings;
    ...
    // CTA candidates: visible link/button with button-ish presentation and
    // short actionable text. Filled background = visually primary.
    const ctas = [];
    for (const el of doc.querySelectorAll('a, button, [role="button"], input[type="submit"]')) {
      if (el.closest('.impeccable-overlay, .impeccable-label, .impeccable-banner, [id^="impeccable-live-"]')) continue;
      if (!isVisible(el)) continue;
      const text = (el.textContent || el.value || '').trim().replace(/\s+/g, ' ');
      if (text.length < 2 || text.length > 45) continue;
      const s = win.getComputedStyle(el);
      const cls = el.getAttribute('class') || '';
      const filled = !!s.backgroundColor && !cssColorIsTransparent(s.backgroundColor);
      const bordered = parseFloat(s.borderTopWidth) > 0 && s.borderTopStyle !== 'none';
      const named = /\b(btn|button|cta)\b/i.test(cls);
      if (!filled && !bordered && !named) continue;
      const r = el.getBoundingClientRect();
      // Skip elements parked outside the viewport box (skip-links, off-canvas
      // drawers): they are not part of the rendered decision surface.
      const onCanvas = r.right > 0 && r.left < vw && r.bottom + scrollY > 0;
      if (!onCanvas) continue;
      ctas.push({ text: text.toLowerCase(), top: r.top + scrollY, bottom: r.bottom + scrollY, filled });
    }

    const pageHeight = Math.max(doc.body.scrollHeight, doc.documentElement.scrollHeight);
    const scrollable = pageHeight > vh * 1.2;

    const aboveFold = ctas.filter(c => c.top >= 0 && c.bottom <= vh);

    // cta-below-fold: CTAs exist, page scrolls, none fully inside the first viewport.
    if (ctas.length > 0 && scrollable && aboveFold.length === 0) {
      const first = ctas.reduce((a, b) => (a.top <= b.top ? a : b));
      findings.push({
        id: 'cta-below-fold',
        snippet: `First CTA "${first.text}" starts at ${Math.round(first.top)}px; fold is ${vh}px`,
      });
    }

    // hero-cta-competition: 3+ distinct filled CTAs above the fold (independent
    // of page height — a short page with three loud choices is still a Hick's
    // law failure).
    const distinctFilled = new Set(aboveFold.filter(c => c.filled).map(c => c.text));
    if (distinctFilled.size >= 3) {
      findings.push({
        id: 'hero-cta-competition',
        snippet: `${distinctFilled.size} distinct filled CTAs above the fold: ${[...distinctFilled].slice(0, 4).map(t => `"${t}"`).join(', ')}`,
      });
    }

    // headline-word-wall: h1 copy runs past ~14 words.
    const h1Text = (h1.textContent || '').trim().replace(/\s+/g, ' ');
    const h1Words = h1Text ? h1Text.split(' ').length : 0;
    if (h1Words > 14) {
      findings.push({
        id: 'headline-word-wall',
        snippet: `h1 is ${h1Words} words: "${h1Text.slice(0, 80)}${h1Text.length > 80 ? '…' : ''}"`,
      });
    }

    // one-word-lines: display heading wrapped into a stack of near-single words.
    for (const heading of doc.querySelectorAll('h1, h2')) {
      if (!isVisible(heading)) continue;
      const r = heading.getBoundingClientRect();
      if (r.top + scrollY > vh * 1.5) continue;
      const s = win.getComputedStyle(heading);
      const fontSize = parseFloat(s.fontSize) || 0;
      if (fontSize < 28) continue;
      const lineHeight = s.lineHeight === 'normal' ? fontSize * 1.2 : parseFloat(s.lineHeight) || fontSize * 1.2;
      const lines = Math.round(r.height / lineHeight);
      const text = (heading.textContent || '').trim().replace(/\s+/g, ' ');
      const words = text ? text.split(' ').length : 0;
      if (lines >= 4 && words > 0 && words / lines < 2.6) {
        findings.push({
          id: 'one-word-lines',
          snippet: `<${heading.tagName.toLowerCase()}> renders ${words} words over ~${lines} lines (${(words / lines).toFixed(1)} words/line) at ${Math.round(fontSize)}px`,
        });
        break; // one instance describes the pattern
      }
    }

    // Media above the fold: img/video/canvas/picture/svg or a url() background
    // covering a meaningful share of the first viewport.
    const foldArea = vw * vh;
    let mediaArea = 0;
    const mediaSelector = 'img, video, canvas, picture, svg';
    const intersectFold = (r) => {
      const top = Math.max(0, r.top + scrollY);
      const bottom = Math.min(vh, r.bottom + scrollY);
      const width = Math.min(r.right, vw) - Math.max(r.left, 0);
      return bottom > top && width > 0 ? (bottom - top) * width : 0;
    };
    for (const el of doc.querySelectorAll(mediaSelector)) {
      if (!isVisible(el)) continue;
      mediaArea += intersectFold(el.getBoundingClientRect());
      if (mediaArea >= foldArea * 0.08) break;
    }
    if (mediaArea < foldArea * 0.08) {
      for (const el of doc.querySelectorAll('*')) {
        if (el.closest('.impeccable-overlay, [id^="impeccable-live-"]')) continue;
        const r = el.getBoundingClientRect();
        if (r.top + scrollY > vh) continue;
        const bg = win.getComputedStyle(el).backgroundImage || '';
        if (!bg.includes('url(')) continue;
        if (!isVisible(el)) continue;
        mediaArea += intersectFold(r);
        if (mediaArea >= foldArea * 0.08) break;
      }
    }
    const hasHeroMedia = mediaArea >= foldArea * 0.08;

    // missing-hero-media: landing-shaped page (h1 + a CTA) with a text-only first viewport.
    if (!hasHeroMedia && ctas.length > 0) {
      findings.push({
        id: 'missing-hero-media',
        snippet: `Above-the-fold media covers ${Math.round((mediaArea / foldArea) * 100)}% of the viewport (threshold 8%)`,
      });
    }

    // hero-viewport-hog: the h1's full-width section runs past ~1.15 viewports without media.
    let hero = h1.parentElement;
    let heroSection = null;
    while (hero && hero !== doc.body) {
      const r = hero.getBoundingClientRect();
      if (r.width >= vw * 0.8 && r.height >= vh * 0.6) { heroSection = hero; break; }
    ...
    if (heroSection) {
      ...
      if (r.height > vh * 1.15) {
        let sectionMedia = 0;
        for (const el of heroSection.querySelectorAll(mediaSelector)) {
          if (!isVisible(el)) continue;
          const mr = el.getBoundingClientRect();
          sectionMedia += Math.max(0, mr.width * mr.height);
          if (sectionMedia >= foldArea * 0.08) break;
        }
        if (sectionMedia < foldArea * 0.08) {
          findings.push({
            id: 'hero-viewport-hog',
            snippet: `Hero section is ${Math.round(r.height)}px tall (${(r.height / vh).toFixed(2)} viewports) with no meaningful media`,
          });
        }
      }
    }
  ...
  // ─── Hover-state legibility (stylesheet-driven, browser only) ──────────────
  // Agents routinely ship `:hover { color: X }` combinations that turn the
  // label invisible on its own background. Same-origin stylesheets are walked
  // for :hover rules that declare color/background; the resulting pair is
  // contrast-checked against the element's resting computed values.
  function checkHoverStylesDOM() {
    const findings = [];
    const win = window;
    const doc = document;
    const customPropMap = buildCustomPropMap(doc);
    const seen = new Set();

    const collectRules = (rules, out) => {
      for (const rule of rules) {
        if (rule.type === 1) out.push(rule); // CSSStyleRule
        else if ((rule.type === 4 || rule.type === 12) && rule.cssRules) collectRules(rule.cssRules, out); // media / supports
      }
    };
    ...
    for (const sheet of doc.styleSheets) {
      ...
      if (rules) collectRules(rules, allRules);
    }

    for (const rule of allRules) {
      const selectorText = rule.selectorText || '';
      if (!selectorText.includes(':hover')) continue;
      const declaredColor = rule.style?.color || '';
      const declaredBg = rule.style?.backgroundColor || '';
      if (!declaredColor && !declaredBg) continue;

      for (const sel of selectorText.split(',').map((s) => s.trim())) {
        if (!sel.includes(':hover')) continue;
        // Only simple self-hover selectors: `.btn:hover`, `a:hover.x`. Skip
        // descendant hovers (`.card:hover .title`) — resolving the affected
        // child reliably needs more machinery than the payoff justifies.
        const withoutHover = sel.replace(/:hover/g, '');
        if (/[\s>+~]/.test(sel.split(':hover')[1] || '')) continue;
        let el = null;
        try { el = doc.querySelector(withoutHover.trim() || '*'); } catch { continue; }
        if (!el) continue;
        const r = el.getBoundingClientRect();
        ...
        const fgRaw = declaredColor || computed.color;
        let bgRaw = declaredBg;
        if (!bgRaw) {
          // Resting effective background: walk up to the first non-transparent.
          let node = el;
          while (node && node !== doc.documentElement) {
            const bc = win.getComputedStyle(node).backgroundColor;
            if (bc && !cssColorIsTransparent(bc)) { bgRaw = bc; break; }
            node = node.parentElement;
          }
          if (!bgRaw) bgRaw = 'rgb(255,255,255)';
        }
        const fg = parseColorResolved(fgRaw, customPropMap);
        const bg = parseColorResolved(bgRaw, customPropMap);
        if (!fg || !bg) continue;
        const ratio = contrastRatio(fg, bg);
        const fontSize = parseFloat(computed.fontSize) || 16;
        const bold = (parseInt(computed.fontWeight, 10) || 400) >= 700;
        const isLarge = fontSize >= WCAG_LARGE_TEXT_PX || (bold && fontSize >= WCAG_LARGE_BOLD_TEXT_PX);
        ...
        if (ratio < need - 0.05) {
          ...
          findings.push({
            id: 'hover-contrast',
            snippet: `${sel} → ${ratio.toFixed(1)}:1 on hover (need ${need}:1) — text ${colorToHex(fg)} on ${colorToHex(bg)}`,
          });
        }
      }
    }
  ...
  // ─── Header / wordmark scale (page-level, browser only) ────────────────────
  function checkHeaderScaleDOM() {
    const findings = [];
    const win = window;
    const doc = document;
    const vh = win.innerHeight;
    if (!vh) return findings;

    // Header candidate: <header>/<nav>/[role=banner] whose box starts in the
    // top 150px of the page.
    let header = null;
    for (const el of doc.querySelectorAll('header, nav, [role="banner"]')) {
      const r = el.getBoundingClientRect();
      if (r.top + (win.scrollY || 0) < 150 && r.height > 0 && r.width > win.innerWidth * 0.6) { header = el; break; }
    }
    ...
    if (hr.height > 128) {
      findings.push({
        id: 'oversized-header',
        snippet: `Header block is ${Math.round(hr.height)}px tall (convention ~56-88px)`,
      });
    }
    for (const logo of header.querySelectorAll('img, svg')) {
      const lr = logo.getBoundingClientRect();
      if (lr.height > 64 && lr.height > hr.height * 0.55) {
        findings.push({
          id: 'oversized-header',
          snippet: `Wordmark/logo renders ${Math.round(lr.height)}px tall inside a ${Math.round(hr.height)}px header (convention ~20-40px)`,
        });
        break;
      }
    }
  ...
  function collectBrowserFindings() {
    ...
    if (structureFindings.length > 0) {
      pageLevelFindings.push(...structureFindings);
      addBrowserFindings(groupMap, document.body, structureFindings);
    }
