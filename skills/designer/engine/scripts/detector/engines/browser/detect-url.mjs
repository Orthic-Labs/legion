...
async function detectUrl(url, options = {}) {
  ...
  if (!externalBrowser) {
    try {
      ...
      // No puppeteer: fall back to driving an installed Chrome/Edge over raw
      // CDP so URL scans still measure the rendered page (the alternative —
      // silently degrading to static scanning — hides every geometry rule).
      const { detectUrlCdp } = await import('./detect-url-cdp.mjs');
      if (!options?.quiet) {
        process.stderr.write('Note: puppeteer not installed — scanning via installed Chrome/Edge over CDP (visual-contrast screenshot lane skipped).\n');
      }
      return detectUrlCdp(url, {
        ...options,
        designSystem: serializeDesignSystemForBrowser(options?.designSystem),
      });
    }
  }
...
async function createBrowserDetector(options = {}) {
  ...
  try {
    ...
    // No puppeteer: hand back a detector that scans each URL over raw CDP
    // against an installed Chrome/Edge (one short-lived headless launch per
    // URL — slower than a shared puppeteer browser, but correct).
    const { detectUrlCdp } = await import('./detect-url-cdp.mjs');
    return {
      browser: null,
      async detectUrl(url, scanOptions = {}) {
        const merged = { ...options, ...scanOptions };
        return detectUrlCdp(url, {
          ...merged,
          designSystem: serializeDesignSystemForBrowser(merged?.designSystem),
        });
      },
      async close() {},
    };
