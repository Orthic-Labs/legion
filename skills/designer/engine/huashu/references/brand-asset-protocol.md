...
0. **图标聚合源（知名数字产品/SaaS/AI 工具首选，命中率最高）**：
   ...
   Save the Simple Icons SVG into `assets/<brand>-brand/logo.svg`, then inspect it before use.
   ```
1. 独立 SVG/PNG 文件 / 官方 brand 页（如 `<brand>.com/brand`、`/press`）：
   ```bash
   Save the official logo file into `assets/<brand>-brand/logo.svg`, then inspect it before use.
   ```
2. 官网 HTML 全文提取 inline SVG：
   ```bash
   Save the official homepage HTML into `assets/<brand>-brand/homepage.html` for local SVG extraction.
   # 然后 grep <svg>...</svg> 提取 logo 节点
   ```
3. **Google favicon 服务（站点真实 mark 兜底，几乎不失败）**：
   ```bash
   Save the favicon image as `logo.png` from the official domain lookup, then inspect it before use.
   ```
