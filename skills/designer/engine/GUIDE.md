...
description: Use when the user wants to design, redesign, shape, critique, audit, polish, clarify, distill, harden, optimize, morph, animate, colorize, extract, or otherwise improve a frontend interface. Covers websites, landing pages, dashboards, product UI, app shells, components, forms, settings, onboarding, and empty states. Handles UX review, visual hierarchy, information architecture, cognitive load, accessibility, performance, responsive behavior, theming, anti-patterns, typography, fonts, spacing, layout, alignment, color, motion, micro-interactions, UX copy, error states, edge cases, i18n, and reusable design systems or tokens. Also use for bland designs that need to become bolder or more delightful, loud designs that should become quieter, live browser iteration on UI elements, or ambitious visual effects that should feel technically extraordinary. Also produces design DELIVERABLES via the folded-in huashu engine — HTML slide decks exportable to editable PPTX/PDF, motion/animation sequences rendered to MP4/GIF, and AI voiceover. Not for backend-only or non-UI tasks.
version: 3.7.1
user-invocable: false
argument-hint: "[craft|shape · audit|critique · animate|bolder|colorize|delight|layout|overdrive|quieter|typeset · morph|clarify|distill · harden|onboard|optimize|polish · init|document|extract|live] [target]"
license: Apache 2.0
---
...
1. Run `node skills/designer/engine/scripts/context.mjs` once per session. If the request names or implies a file, route, or app inside a monorepo, infer the concrete path and run `node skills/designer/engine/scripts/context.mjs --target <path>` instead. If you've already seen its output in this conversation, do not re-run it. The script either prints the project's PRODUCT.md (and DESIGN.md when present) as a markdown block, or tells you it's missing. Follow whatever it prints. **If it reports `NO_PRODUCT_MD`, stop and follow `reference/init.md` before doing anything else.** If the output ends with an `UPDATE_AVAILABLE` directive, follow it (ask the user once about updating, then continue). It never blocks the current task.
...
5. **If the project is brand-new (no existing CSS tokens / theme / committed brand colors found in step 3)**, run `node skills/designer/engine/scripts/palette.mjs` to receive a brand seed color and composition guidance. This is the anchor for your primary brand color. Compose the rest of the palette (bg, surface, ink, accent, muted) around it per the script's instructions. Use OKLCH throughout. **Skip this step only if step 3 found committed brand colors in existing tokens; in that case identity-preservation wins.**
...
### Working method (folded from huashu-design)

Four disciplines apply to every task and override speed:

- **Fact-verification first (Principle #0 — highest priority).** If the task names a specific product, technology, brand, or event ("DJI Pocket 4", "Gemini 3 Pro"), your FIRST action is a `WebSearch` to confirm it exists, its current version, and its specs. Never design from training-corpus memory — ~10s of search beats 1-2h of rework on a wrong assumption.
- **Core Asset Protocol.** When a real brand is in scope, do not guess its look. Ask for assets, then search official channels (`<brand>.com/brand` · `/press` · product pages · launch films), download by asset type (logo SVG → product shots → UI screenshots), verify + grep real color hex, and freeze to a `brand-spec.md`. Real assets — never CSS silhouettes standing in for product shots. Full protocol: `huashu/references/brand-asset-protocol.md`.
- **Junior-Designer mode (default).** Send the full question set in one batch and wait for all answers. Write assumptions + placeholders + reasoning into the markup, show the user early (even gray blocks), then fill real content → variations → tweaks, showing at each step. Eyeball the rendered result in a browser before delivery.
- **Variations, not a single "final answer."** Offer differentiated options and let the user choose. For a too-vague brief, enter the Design Direction Advisor: propose 3 directions from different schools, render quick demos, let the user pick before committing.
...
- **Tiny uppercase tracked eyebrow above every section.** The 2023-era kicker (small all-caps text with wide tracking, "ABOUT" "PROCESS" "PRICING" above each heading) is now the saturated AI scaffold; it appears on 55-95% of generations regardless of brief, which is the definition of a tell. One named kicker as a deliberate brand system is voice; an eyebrow on every section is AI grammar. Choose a different cadence. The copy half of the same tell: furniture whose text restates its neighbor (kicker ≈ h2, deck ≈ headline — "Available now / four working paths" over "Four working paths, available now."). Write each label to carry information the adjacent line doesn't, or write no label — headline + deck carry. A kicker is earned only when it adds a real axis (e.g., a contrasting pair like AVAILABLE TODAY / NOT INCLUDED TODAY).
...
| `morph [target]` | Fix | Morph for different devices and screen sizes | reference/morph.md |
...
### Deliverables engine (huashu)

The `deck` / `motion` / `video` / `voiceover` commands route into the folded-in **huashu** sub-engine (`huashu/GUIDE.md` + `huashu/references/` + `huashu/scripts/` + `huashu/assets/`). It turns a design into a shippable artifact: HTML slides exported to editable PPTX/PDF (`huashu/scripts/export_deck_pptx.mjs`), a Stage+Sprite motion timeline rendered to MP4/GIF (`huashu/scripts/render-video.js`), and AI voiceover mixed in (`huashu/scripts/narrate-pipeline.mjs`). Device frames (iOS / Android / macOS / browser) live in `huashu/assets/`. When one of these commands is invoked, read `huashu/GUIDE.md` first for its 7-phase flow, then the matching `huashu/references/*.md`. **Setup note:** the huashu export/render/TTS scripts need `node` + Python + `ffmpeg` (and a TTS API key in a local `.env` for voiceover); the design phases work without them — only the final export step needs that toolchain.

### Routing rules

1. **No argument**: the user is asking "what should I do?" Make the menu context-aware instead of static. Setup has already run `context.mjs`; if that reported `NO_PRODUCT_MD` you are already in init (setup), so finish that and skip this. Otherwise run `node skills/designer/engine/scripts/context-signals.mjs` once and read its JSON, then lead with the **2-3 highest-value next commands**, each with a one-line reason pulled from the signals, followed by the full menu (the table above, grouped by category). **Never auto-run a command; the recommendation is a suggestion the user confirms.**

   Reason over the signals; there is no score to obey:
   - `setup.hasDesign` false while `setup.hasCode` true → `document` (capture the visual system).
   - `critique.latest` is `null` → the project has never been critiqued; for a set-up project with a real surface, offering `/designer critique <surface>` is a strong default.
   ...
   **If `scan.targets` is non-empty, run `node skills/designer/engine/scripts/detect.mjs --json <scan.targets joined by spaces>` once** (the bundled detector over local files: no network, no npx). `scan.via` tells you what they are: `git-changes` (the markup/style files in your dirty tree, the most relevant set), `source-dir` (e.g. `src`, `app`), `html`, or `root`. Fold the hits into your picks: many quality / contrast hits → `audit` or `polish`; a specific slop family → the matching command (gradient text or eyebrows → `quieter` / `typeset`, flat or gray palette → `colorize`, and so on). It's a real, current signal that beats guessing. If detect errors or the tree is large and slow, skip it and recommend the user run `audit` themselves; never block the suggestion on it.

   Keep it to 2-3 pointed picks with the exact command to type. The menu stays the fallback; the recommendation is the lede.
2. **First word matches a command** (table above or `hooks`): load its reference file and follow its instructions. Everything after the command name is the target.
