...
**Output**: A design brief that can be handed off to /designer craft, or directly to /designer for freeform implementation. When visual direction probes are used, the images are supporting artifacts, not the primary output.
...
Once confirmed, the brief is complete. The user can now hand it to /designer, or use it to guide any other implementation approach. (If the user wants the full discovery-then-build flow in one step, they should use /designer craft instead, which runs this command internally.)
