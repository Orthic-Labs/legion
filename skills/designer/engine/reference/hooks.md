...
2. Invoke the admin script and pass the user's output through verbatim:
   ...
   ```bash
   legion script designer/hook-admin <action> [args...]
   ```
...
```bash
legion script designer/hook-admin ignore-value overused-font Inter --shared --reason "User confirmed Inter is intentional"
```

Example intentional motion exception:

```bash
legion script designer/hook-admin ignore-value bounce-easing bounce-ball --shared --reason "User confirmed ball bounce animation is intentional"
```

Example whole-rule font exception:

```bash
legion script designer/hook-admin ignore-rule overused-font --all-values --reason "User asked to ignore overused fonts generally"
```

Example file-scoped exception:

```bash
legion script designer/hook-admin ignore-file "src/legacy/Card.tsx"
```
