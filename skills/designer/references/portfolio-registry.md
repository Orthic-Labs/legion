...
|---|---|---|---|---|---|---|
| **ViewRight** | Editorial / print | Warm paper (light) | Reading serif (Source Serif / Spectral) | Oxblood `#7A2E2E` | Two-column: source → rendered | Ink-settle / page-render sweep |
| **MailRight** | Swiss / utilitarian | Crisp off-white (light) | Grotesque (Söhne / Suisse Int'l) | Signal blue `#1B5FFF` | Strict baseline grid + kbd chips | Keyboard-snap micro-motion |
| **HeardRight** | Sonic / acoustic | Night (dark) | Humanist sans (used as "voice") | Signal teal `#16B8A6` on indigo | Waveform-as-spine | Audio-reactive transcribe |
| **CodeRight** | Runtime schematic / terminal | Warm near-black (dark) | Mono + condensed (Spline Mono / Saira) or display serif (Instrument) | Copper `#C8794A` (+ sage = verified) | Runtime diagram → RUN panel | Data-flow trace + arbiter-reject pulse |
...
> ⚠ **Drift to fix:** HeardRight's locked brand (`heardright/brand.md`) currently uses ink/paper/**copper**
> — the SAME warm-neutral+copper family as CodeRight. That overlap is the exact "AI tell." The registry
> re-points HeardRight to a **night + teal acoustic** identity to break the twinning. Reconcile
> `brand.md` when HeardRight is redesigned (the approving human's call — this changes a locked brand).
...
### ViewRight — "the render reveal"
Split hero. **Left:** raw source (plain markdown / code / a messy file), monospace, deliberately
unstyled. **Right:** the same content as ViewRight *renders* it — typeset, beautiful, readable.
...
### CodeRight — "the runtime, not the agent" (CORRECTED 2026-06-02 — the moat)
**The moat is NOT an approval gate — every harness waits for you. CodeRight is a self-improving
orchestration RUNTIME; the model is a swappable worker, the runtime is the product** (source:
`coderight/docs/architecture/00-BREWS.md`). The hero must SHOW what no single-model harness can:
...
BANNED framing for CodeRight: "supervise / approve / one attention color / waits for you" — table
...
|---|---|---|---|---|
| Damned Designs | Dark premium EDC | Ink / beige | Copper `#B87333` | (CodeRight is also copper — DD is product/e-com, keep its Cormorant serif + beige to separate) |
| Rotten Hand | Honest slow-fashion | Warm light | Muted rose `#b07a84` | MailRight blue / ViewRight oxblood |
| Toxic Sundae | Counter-culture streetwear | Black | Toxic green `#39FF14` | n/a (already loud + distinct) |
