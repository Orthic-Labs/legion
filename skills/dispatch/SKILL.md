...
5. Bind worker boundary in every lane: workers inspect required inputs & edit only their exact allowlist; they do not run Cargo, tests, builds, generators, installs, commits, pushes, merges, or expensive checks. Record intended checks for integration owner.
6. Bind integration owner as sole merger & checkpoint runner: reconcile changed paths, merge lane outputs, run focused checks plus Cargo/tests/builds/expensive checkpoints when required, & own final evidence. Integrator may not repair lane-owned files; send repair to owning lane or redesign packet before execution.
