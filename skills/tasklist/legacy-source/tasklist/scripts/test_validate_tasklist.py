...
def run(*args: object) -> subprocess.CompletedProcess[str]:
    return subprocess.run(
        ["py", "-3.11", *(str(arg) for arg in args)],
        cwd=ROOT.parents[2],
        text=True,
        capture_output=True,
        check=False,
...
def prepare(name: str) -> tuple[Path, Path]:
    ...
    text = text.replace(str(EXAMPLE).replace("\\", "/"), str(tasklist).replace("\\", "/"))
    text = text.replace(str(ROUTE).replace("\\", "/"), str(route).replace("\\", "/"))
    text = text.replace(
        str(ROUTE_RECEIPT).replace("\\", "/"),
        str(route_receipt).replace("\\", "/"),
    )
