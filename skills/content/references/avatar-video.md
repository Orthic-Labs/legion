...
Keep voice, avatar identity, lipsync, and final video rendering as separate decisions. Use SampleApp/current pipelines.

## Current Default

- Talking head from face image plus external audio: current InfiniteTalk-style route.
- Custom voice: generate or select a WAV first through SampleApp Voice Studio/current TTS route.
...
If the required key is missing from SampleApp model metadata, do not fake the route. Report the missing model and offer the current InfiniteTalk/current pipeline alternative.
...
- Do not put HeyGen-specific settings outside the normal SampleApp form-contract rendering.
