...
Use GenRight Video Studio/current video pipeline. This branch creates a storyboard and normal video preflight seed, not a provider-specific request.
...
- Normal GenRight Video Studio preflight payload
...
If making a storyboard sheet prompt, generate one sheet concept, not separate provider calls per panel. The sheet is a planning artifact; final rendering still goes through GenRight/current pipeline preflight and eyes review.

## Model Defaults

- Product/social anchored motion: prefer the cheap draft route exposed in GenRight/current contracts.
...
- Do not call connector/provider tools from this skill. Seed a GenRight/current-pipeline preflight instead.
