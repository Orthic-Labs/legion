...
## Backend lock (HARD — read `D:/Claude/.claude/rules/video-pipeline.md` first)
...
4. **Adrian taste / eyes gate.** `tools/lib/human-eyes-gate.mjs` halts the pipeline at every taste-critical artifact (storyboard frames, per-clip QA, final cut) until Adrian explicitly approves. AI suggests, jurors score — **only Adrian approves**. No agent can approve on his behalf.
...
6. **Per-clip dual-juror QA → Adrian eyes-gate → assemble → final dual-juror QA → final eyes-gate.** All enforced; do not present output as deliverable until the eyes-gate records APPROVE.
