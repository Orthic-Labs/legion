...
Bundled workflow for the operator's KDP/Etsy publishing lane: brainstorm the book, validate the market, write or review the manuscript, enforce brand voice, prepare listing copy, and check KDP production risk.
...
4. Treat the operator as final owner of taste, but do not dump preventable review work on him. The skill should catch mechanical, cultural, policy, and quality issues first.
...
6. **Review:** run manuscript/art QA and, when the operator explicitly requests an external panel, `/covenant` with the correct non-code rubric.
...
- Taste/cultural/positioning calls only the operator can own.
Checked:
