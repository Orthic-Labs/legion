...
**Live-URL gate (MANDATORY):** When the flow/page/paywall/popup/signup exists as a live URL, localhost route, or app preview, the shared `qa` skill chain is required — not optional — before issuing any visual or interaction verdict. Use the project `qa:browser` contract when available; otherwise use the `qa` skill's `legion-skill://qa/scripts/qa-functional.mjs` for click/hover/type/assert flows and `legion-skill://qa/scripts/qa-shot.mjs` for viewport screenshots. A CRO verdict issued without browser evidence for a live URL is invalid. Do not use foreground desktop screenshots for routine CRO QA.
