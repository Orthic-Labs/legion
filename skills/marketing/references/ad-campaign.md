...
node -e "import('@orthic-labs/legion/auto-jury').then(m=>m.runAutoJury({
  ...
  context: { brand: '<brand-code or N/A>', notes: 'plan-ad-campaign output', rubricFlags: {} },
  failHard: true
...
When the ad-campaign plan is approved: hand off to `/ads` for execution + ongoing audit. Pass the campaign brief, budget split, platform selection, and creative direction as the `/ads` input. `/ads` owns bid strategy, creative testing cadence, and ROAS/CPA optimization — `/marketing strategy` ends here.
