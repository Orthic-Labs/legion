...
**Pull keywords + cluster into topical groups mapped to pages.**

1. **Discovery** — pull 100-500 candidate keywords via DataForSEO/Ahrefs (volume, difficulty, intent, SERP features)
2. **Cluster** — group by:
   - **Topical similarity** (semantic — "running shoes" + "best trainers" + "sneakers for running")
   - **Search intent** (informational / commercial / transactional / navigational)
   - **Funnel stage** (awareness → consideration → decision)
   - **SERP overlap** (queries that share the same top-10 results = same cluster, same page)
3. **Map cluster → page** — each cluster becomes ONE pillar page or one cluster of supporting posts. Don't fragment a cluster across multiple pages (cannibalization).
4. **Cluster scoring** — for each cluster: total volume, avg difficulty, commercial value, content effort. Prioritize high-value × low-difficulty first.
5. **Output**: `KEYWORD-CLUSTERS.md` with cluster name → page URL → primary keyword + 5-15 supporting keywords + intent + funnel stage.

**Anti-patterns**:
- One page per keyword (Google wants topic-level pages, not keyword-level)
- Pulling kw without checking SERP intent (you'll write the wrong content type)
- Ignoring SERP overlap (two pages targeting overlapping kw = cannibalization)
- Clustering by language alone, not search intent

### 5. Content Strategy
- Content gaps vs competitors (use cluster output from §4)
...
- Content calendar with priorities (driven by cluster priority scores)
...
node -e "import('file:///D:/workspace/tools/lib/auto-jury.mjs').then(m=>m.runAutoJury({
  ...
  context: { brand: '<DD|RH|HR|TS or N/A>', notes: 'plan-seo output', rubricFlags: {} },
  failHard: true
