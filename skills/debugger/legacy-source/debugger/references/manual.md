...
- If 3+ attempted fixes fail, stop — **you are in a local minimum, looping your own failed logic.** The
  ...
  represent reality) → hand to `/architect` to design the fix, then resume here at Fix + verify; (b) the
  diagnosis itself is uncertain / multiple hypotheses fit equally → escalate to `/council code` for
...
External review is explicit opt-in. Run `/council code <changed-file-or-diff>` only when Adrian asks
...
`.claude/rules/ssh-server-access.md`.
