...
**What this is:** the method manual for the Debugger diagnosis capability (the "establish
material facts, root cause" craft). Recovered verbatim from git history — retained from the
original review craft. Debugger owns this method; it does not route through Sage. Sage attaches
only when evidence exposes a material unresolved semantic/ownership/acceptance decision.
