...
5. If Adrian explicitly says `inline`, return packet inline instead of writing file.
6. Explain problem + Adrian's intent simply enough for zero-context third-party agent, while retaining exact failure evidence, current state, constraints, attempted approaches, & requested response.
...
9. Validate exact output before delivery:
   ...
   py -3.11 D:/Claude/tools/skills/council/scripts/validate-external-review-packet.py <packet.md>

   # Explicit inline mode: validate draft bytes before pasting exact content
   py -3.11 D:/Claude/tools/skills/council/scripts/validate-external-review-packet.py <draft.md> --inline
   ```
...
- **`USER_INTENTION` carries Adrian's request VERBATIM, never the implementer's restatement.** The packet is written by the agent whose work is under review, so a paraphrased objective launders the implementer's framing into the gate — the panel then judges "did this meet the goal as I described it," which it always did. Quote the original ask; put the restatement in `SUCCESS_CRITERIA` where it is visibly the author's claim and can be disputed.
...
- Adrian's eyes remain the visual/taste authority.
...
- After 2 complete loops, **stop**: the phase ships on its evidence, or a single named hard blocker
  escalates to Adrian. Do not run a 3rd panel on the same phase.
...
- Run this external workflow only when Adrian explicitly opts in under the workspace review-routing rule.
