...
Free, local-first voice synthesis studio for private, zero-cost voice generation. The open-source alternative to ElevenLabs.
...
**Install:** Use the desktop apps for macOS and Windows at [voicebox.sh](https://voicebox.sh). If building from source, use a reviewed pinned local checkout and run setup only after inspecting the project instructions.
