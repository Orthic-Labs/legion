...
5. Native foreground screenshots only when Adrian explicitly asks or native-window behavior is genuinely required.
...
- Adrian explicitly asks for `/audit-visual`, visual jury, or external review.
...
- **Regression context (when the audit is triggered post-change).** A finding is not automatically a
  ...
  condition* → **Note** severity; do not block the ship for it unless Adrian explicitly scoped it in.
...
- **Structure floor (websites/landing pages):** any measured `structure` detector hit —
  ...
  Adrian can waive one, explicitly, per finding. (`missing-hero-media` is advisory: the lens
