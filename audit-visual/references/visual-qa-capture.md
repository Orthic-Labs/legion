...
1. **Local `/qa` skill and project QA harness.**
   ...
   - Use `D:\Claude\tools\skills\qa\scripts\qa-shot.mjs` for viewport screenshots and `D:\Claude\tools\skills\qa\scripts\qa-functional.mjs` for hover/click/type/key/assert flows.
...
4. **Native Tauri/WebView2 foreground QA only by explicit request.**
   ...
   - For ScrapeRight-style Tauri apps, use the documented script such as `.\scripts\dev.ps1`.
...
node D:\Claude\tools\skills\qa\scripts\qa-shot.mjs --url $url --out .cache\qa-shots\current\app-default.png
node D:\Claude\tools\skills\qa\scripts\qa-functional.mjs --url $url --actions .cache\qa-actions.json
```

If the project provides a dedicated screenshot script, use that instead of improvising. For ScrapeRight, follow `docs/QA.md` and the project `AGENTS.md`: routine visual QA uses the headless Vite QA harness, not foreground Tauri/WebView2.
