...
test('Sage task budget seals exact contract, task & scope bindings immutably', () => {
  ...
  const ring = generateTestKeyRing(); const root = mkdtempSync(path.join(os.tmpdir(), 'task-budget-')); const store = new TaskBudgetSealStore({ root, keyRing: ring, clock: () => '2026-08-12T00:00:00Z' }); const assertion = { authority: 'sage', assertedBy: 'host', verificationMethod: 'capability-signature', perMessage: true };
  assert.equal(store.seal({ contract, task, authorityAssertion: assertion }).taskId, 'T-1'); const file = path.join(root, readdirSync(root)[0]); const forged = JSON.parse(readFileSync(file)); forged.activeTimeCapMs = 99; writeFileSync(file, JSON.stringify(forged)); assert.throws(() => store.require('EC-1', 'T-1'), /MAC/); task.ownScope = ['changed']; assert.throws(() => store.seal({ contract, task, authorityAssertion: assertion }), /binding mismatch/);
});
