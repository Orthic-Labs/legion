...
const PURPOSES = new Set(['completion-claim','assurance-request','assurance-countersign']);
const ROLES = new Set(['legion','alchemist','sage','covenant']);
...
export class AuthorityInvocationProofIssuer {
  ...
  issue({ ledger, binding, purpose, role }) {
    if (!this.ledgerStore || !ledger || !binding || !PURPOSES.has(purpose) || !ROLES.has(role)
