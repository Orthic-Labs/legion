...
export class ContractSealStore {
  ...
  require(contract) { const record = this.get(contract.contractId, contract.version); if (!record) fail('ARC_NO_CONTRACT', 'contract seal missing'); if (record.sourceRevision !== contract.sourceRevision || record.contractDigest !== digestValue(contract) || !same(record.contract, contract)) fail('ARC_CONTRACT_VERSION_MISMATCH', 'contract seal identity mismatch'); return record; }
