...
export function applyArchitectureEvent(state, event) {
  ...
  else if (event.event_type === 'INVALIDATION_RECORDED') { payloadKeys(payload,['scope','cause','root_evidence','decision_ids'],'INVALIDATION_RECORDED'); if (!INVALIDATION_TARGET[payload.scope] || typeof payload.cause !== 'string' || !payload.cause) fail('invalidation requires cause and scope'); if (payload.scope === 'ROOT' && !payload.root_evidence) fail('root invalidation requires root evidence'); const decision_ids = payload.decision_ids == null ? [] : checkedIds(payload.decision_ids, 'decision_ids'); if (decision_ids.some((id) => !next.decision.items.some((decision) => decision.id === id))) fail('invalidation decision must exist'); if (payload.scope !== 'PATCH') next.task.architecture_status = INVALIDATION_TARGET[payload.scope]; next.convergence.invalidation = { scope:payload.scope, cause:payload.cause, root_evidence:payload.root_evidence ?? null, decision_ids }; }
  ...
  else if (event.event_type === 'ACCEPTANCE_RESULT_RECORDED') upsert(next.acceptance_ledger.items ?? (next.acceptance_ledger.items = []), payload);
